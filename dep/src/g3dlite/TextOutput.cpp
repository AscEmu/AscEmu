...
void TextOutput::convertNewlines(const std::string& in, std::string& out) {
    ///\todo can be significantly optimized in cases where single characters are copied in order by walking through the array and copying substrings as needed.
...
void TextOutput::wordWrapIndentAppend(const std::string& str) {
    ///\todo keep track of the last space character we saw so we don't have to always search.
    ...
    if ((option.wordWrap == Settings::WRAP_NONE) ||
        ...
        ///\todo optimize for strings without newlines to add multiple characters.
