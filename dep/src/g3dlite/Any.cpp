...
namespace G3D
{
    ...
    void Any::beforeRead() const
    {
        if (isPlaceholder())
        {
            // Tried to read from a placeholder--throw an exception as if the original operator[] had failed.
            KeyNotFound e;
            ...
            e.message = "This exception may have been thrown later than the actual operator[] invocation.";
    ...
    bool Any::operator==(const Any& x) const
    {
        ...
        switch (m_type)
        {
            ...
            case TABLE:
            {
                ...
                for (Table<std::string, Any>::Iterator it1 = cmptable.begin(), it2 = xcmptable.begin(); it1 != cmptable.end() && it2 != xcmptable.end(); ++it1, ++it2)
                {
                    if (*it1 != *it2)
                    {
                        return false;
    ...
    ///\todo if the output will fit on one line, compress tables and arrays into a single line
