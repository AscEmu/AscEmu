        ...
        if (tf)
        {
            ...
            for (uint32 i=0; i<numSpawns && result; ++i)
            {
                ...
                if (result)
                {
                    ...
                    WorldModel* model = vm->acquireModelInstance(iBasePath, spawn.name);
                    if (!model)
                        printf("StaticMapTree::LoadMapTile() : could not acquire WorldModel pointer [%u, %u]", tileX, tileY);
                    ...
                    if (fread(&referencedVal, sizeof(uint32), 1, tf) == 1)
                    {
                        ...
                        else
                        {
                            ...
                            if (iTreeValues[referencedVal].ID != spawn.ID)
                                DEBUG_LOG("StaticMapTree::LoadMapTile() : trying to load wrong spawn in node");
                            else if (iTreeValues[referencedVal].name != spawn.name)
                                DEBUG_LOG("StaticMapTree::LoadMapTile() : name collision on GUID=%u", spawn.ID);
#endif
                        }
                    }
                    else
                    ...
                    if (result)
                    {
                        ...
                        else
                        {
                            if (!iLoadedSpawns.count(referencedNode))
                                printf("StaticMapTree::UnloadMapTile() : trying to unload non-referenced model '%s' (ID:%u)", spawn.name.c_str(), spawn.ID);
