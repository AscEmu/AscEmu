...
struct DynamicTreeIntersectionCallback_WithLogger
{
    ...
    {
        logger.debug("DynamicTreeIntersectionCallback_WithLogger : Dynamic Intersection log");
    }
    bool operator()(const G3D::Ray& r, const GameObjectModel& obj, float& distance)
    {
        logger.debug("DynamicTreeIntersectionCallback_WithLogger : testing intersection with %s", obj.name.c_str());
        bool hit = obj.intersectRay(r, distance, true, phase_mask);
        if (hit)
        {
            did_hit = true;
            logger.debug("DynamicTreeIntersectionCallback_WithLogger : result: intersects");
        }
        return hit;
    }
    bool didHit() const { return did_hit;}
