...
void dump_as_sql()
{
    if (COLUMN_COUNT != dbc.getFieldCount())
    {
    ...
    FILE *fsql = fopen("dbc_taxi_path_node.sql", "w");

    //drop table if already exist
    fprintf(fsql, "%s", "DROP TABLE IF EXISTS `dbc_taxi_path_node`;\n");

    //create the table
    fprintf(fsql, "%s", "CREATE TABLE dbc_taxi_path_node (\n");
    ...
    for (unsigned int j = 0; j < dbc.getRecordCount(); j++)
    {
        ...
        {
            fprintf(fsql, "%s", "INSERT INTO dbc_taxi_path_node (");
            for (int i = 0; i < COLUMN_COUNT - 1; i++)
                if (sql_translation_table[i][2][0] == '0')
                    fprintf(fsql, "`%s`,", sql_translation_table[i][1]);
            fprintf(fsql, "`%s`) values \n", sql_translation_table[COLUMN_COUNT - 1][1]);
            fprintf(fsql, " (");
        }
        else
            fprintf(fsql, ",(");
...
void dump_as_sql2()
{
    if (COLUMN_COUNT2 != dbc.getFieldCount())
    {
        printf("error,column counts do not match update code for latest dbc format\n");
        exit(1);
    }
    ...
    FILE *fsql = fopen("dbc_taxi_path.sql", "w");

    //drop table if already exist
    fprintf(fsql, "%s", "DROP TABLE IF EXISTS `dbc_taxi_path`;\n");

    //create the table
    fprintf(fsql, "%s", "CREATE TABLE dbc_taxi_path (\n");

    for (int i = 0; i < COLUMN_COUNT2; i++)
        if (sql_translation_table2[i][2][0] == '0')
        {
            if (sql_translation_table2[i][0][0] == 'u')
                fprintf(fsql, "\t `%s` INT (11) UNSIGNED DEFAULT '0' NOT NULL,\n", sql_translation_table2[i][1]);
            else if (sql_translation_table2[i][0][0] == 'i')
                fprintf(fsql, "\t `%s` INT (11) DEFAULT '0' NOT NULL,\n", sql_translation_table2[i][1]);
            else if (sql_translation_table2[i][0][0] == 'f')
                fprintf(fsql, "\t `%s` FLOAT DEFAULT '0' NOT NULL,\n", sql_translation_table2[i][1]);
            else if (sql_translation_table2[i][0][0] == 's')
                fprintf(fsql, "\t `%s` VARCHAR(2000),\n", sql_translation_table2[i][1]);
        }
    ...
    for (unsigned int j = 0; j < dbc.getRecordCount(); j++)
    {
        ...
        if ((j % SQL_INSERTS_PER_QUERY) == 0)
        {
            fprintf(fsql, "%s", "INSERT INTO dbc_taxi_path (");
            for (int i = 0; i < COLUMN_COUNT2 - 1; i++)
                if (sql_translation_table2[i][2][0] == '0')
                    fprintf(fsql, "`%s`,", sql_translation_table2[i][1]);
            fprintf(fsql, "`%s`) values \n", sql_translation_table2[COLUMN_COUNT2 - 1][1]);
            fprintf(fsql, " (");
        }
        else
            fprintf(fsql, ",(");
