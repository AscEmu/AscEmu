...
void PatchMgr::initialize()
{
    ...
    {
        LOGGER.failure("No patches found.");
        return;
    }
    ...
    while (filecount--)
    {
        ...
        if (read_fd < 0)
        {
            LOGGER.failure("Cannot open %s", Buffer3);
            continue;
        }

        if (fstat(read_fd, &sb) < 0)
        {
            LOGGER.failure("Cannot stat %s", Buffer3);
            close(read_fd);
            continue;
        }

        LOGGER.info("PatchMgr : Found patch for b%u locale `%s` (%u bytes).", srcversion, locality, static_cast<uint32_t>(sb.st_size));
        pPatch = new Patch;
        size = sb.st_size;
        pPatch->FileSize = size;
        pPatch->Data = new uint8[size];
