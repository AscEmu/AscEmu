...
void AuthSocket::HandleChallenge()
{
    ...
    switch (client_build)
    {
        ...
        case 18414:
        {
            LOG_DEBUG("Client with valid build %u connected", (uint32_t)client_build);
        }break;
        default:
        {
            LOG_DEBUG("Client %s has unsupported game version. Clientbuild: %u", GetRemoteIP().c_str(), (uint32_t)client_build);
