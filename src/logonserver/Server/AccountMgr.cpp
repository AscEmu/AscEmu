...
{
    logger.info("AccountMgr : Started precaching accounts...");
    m_reloadThread = nullptr;
    m_reloadTime = reloadTime;

    reloadAccounts(true);

    logger.info("AccountMgr : loaded %u accounts.", static_cast<uint32_t>(getCount()));
...
{
    logger.info("AccountMgr : Stop Manager...");
...
void AccountMgr::addAccount(Field* field)
{
    ...
    {
        account->Banned = 0;
        logger.debug("Account %s's ban has expired.", accountName.c_str());
    ...
    {
        account->Muted = 0;
        logger.debug("Account %s's mute has expired.", accountName.c_str());
    ...
    {
        logger.failure("Account `%s` has incorrect number of bytes in encrypted password! Disabling.", accountName.c_str());
...
void AccountMgr::updateAccount(std::shared_ptr<Account> account, Field* field)
{
    ...
    {
        logger.failure(" >> deleting duplicate account %u [%s]...", id, accountName.c_str());
...
void AccountMgr::reloadAccounts(bool silent)
{
    ...
    if (!silent)
        logger.info("[AccountMgr] Reloading Accounts...");
    ...
    if (!silent)
        logger.info("[AccountMgr] Found %u accounts.", static_cast<uint32_t>(_accountMap.size()));
