...
Socket::Socket(SOCKET fd, uint32 sendbuffersize, uint32 recvbuffersize) : m_fd(fd), m_connected(false),    m_deleted(false), m_writeLock(0)
{
    ...
    logger.debug("Created Socket %u", m_fd);
}
...
void Socket::_OnConnect()
{
    ...
    logger.info("Socket::_OnConnect called");
...
void Socket::Disconnect()
{
    ...
    logger.info("Socket::Disconnect on socket %u", m_fd);
...
void Socket::Delete()
{
    ...
    logger.debug("Socket::Delete() on socket %u", m_fd);
