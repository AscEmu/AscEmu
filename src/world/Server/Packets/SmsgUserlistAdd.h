...
{
    class SmsgUserlistAdd : public ManagedPacket
    {
    public:
        uint32_t channelId;
        ...
        SmsgUserlistAdd() : SmsgUserlistAdd(0, 0, 0, "", 0)
        {
        }

        SmsgUserlistAdd(uint32_t channelId, uint8_t channelFlags, uint8_t memberFlags, std::string channelName, uint64_t guid) :
            ManagedPacket(SMSG_USERLIST_ADD, 0),
