...
void WorldSession::handleSpellClick(WorldPacket& recvPacket)
{
    ...
    if (creatureTarget->RemoveAura(lightWellCharges))
    {
        ...
        if (!_player->hasAurasWithId(lightWellRenew))
        {
            ...
            else
            {
                ...
                LOGGER.failure("Spellclick packet received for creature %u but there is no spell associated with it.", creatureTarget->getEntry());
                return;
            }
    ...
    if (spellClickData != nullptr)
    {
        ...
        {
            LOGGER.failure("NPC ID %u has spell associated on SpellClick but spell id %u cannot be found.", creatureTarget->getEntry(), spellClickData->SpellID);
            return;
        }
...
void WorldSession::handleCastSpellOpcode(WorldPacket& recvPacket)
{
    ...
    if (spellInfo == nullptr)
    {
        LOGGER.failure("Unknown spell id %u in handleCastSpellOpcode().", srlPacket.spell_id);
        return;
    }
    ...
    if (!_player->HasSpell(srlPacket.spell_id))
    {
        ...
        LOGGER.info("WORLD: Player %u tried to cast spell %u but player does not have it.", _player->getGuidLow(), srlPacket.spell_id);
        return;
    }
    ...
    if (spellInfo->isPassive())
    {
        ...
        LOGGER.info("WORLD: Player %u tried to cast a passive spell %u, ignored", _player->getGuidLow(), srlPacket.spell_id);
        return;
    }
...
void WorldSession::handlePetCastSpell(WorldPacket& recvPacket)
{
    ...
    {
        LOGGER.failure("Received opcode but player %u has no pet.", _player->getGuidLow());
        return;
    }
    ...
    if (petUnit == nullptr)
    {
        LOGGER.failure("Pet entity cannot be found for player %u.", _player->getGuidLow());
        return;
    }
    ...
    {
        LOGGER.failure("Pet doesn't belong to player %u", _player->getGuidLow());
        return;
    }
...
void WorldSession::handleCancelTotem(WorldPacket& recvPacket)
{
    ...
    {
        LOGGER.failure("Player %u tried to cancel totem from out of range slot %u, ignored.", _player->getGuidLow(), totemSlot);
        return;
    }
