...
void WorldSession::HandleLootMoneyOpcode(WorldPacket& recv_data)
{
    ...
    if (!_player->InGroup())
    {
        if (money)
        {
            ...
            if (sWorld.settings.goldSettings.isCapEnabled && (GetPlayer()->GetGold() + money) > sWorld.settings.goldSettings.limitAmount)
            {
    ...
    else
    {
        ...
        if (party)
        {
            ...
            {
                // Check they don't have more than the max gold
                if (sWorld.settings.goldSettings.isCapEnabled && ((*itr2)->GetGold() + share) > sWorld.settings.goldSettings.limitAmount)
                {
...
void WorldSession::HandleWhoOpcode(WorldPacket& recv_data)
{
    ...
    while (itr != iend && sent_count < 49)   // WhoList should display 49 names not including your own
    {
        ...
        if (!sWorld.settings.serverSettings.showGmInWhoList && !HasGMPermissions())
        {
        ...
        // Team check
        if (!gm && plr->GetTeam() != team && !plr->GetSession()->HasGMPermissions() && !sWorld.settings.interfactionSettings.isInterfactionMiscEnabled)
            continue;
...
void WorldSession::HandleLogoutRequestOpcode(WorldPacket& recv_data)
{
    ...
    if (pPlayer)
    {
        ...
        if (GetPermissionCount() == 0)
        {
            ...
            if (pPlayer->m_isResting || pPlayer->GetTaxiState() || sWorld.settings.optionalSettings.enableInstantLogoutForAccessType == 2)
            {
        ...
        if (GetPermissionCount() > 0)
        {
            if (pPlayer->m_isResting || pPlayer->GetTaxiState() || sWorld.settings.optionalSettings.enableInstantLogoutForAccessType > 0)
            {
                //Logout on NEXT sessionupdate to preserve processing of dead packets (all pending ones should be processed)
                SetLogoutTimer(1);
                return;
        ...
        case GAMEOBJECT_TYPE_FISHINGNODE:
        {
            ...
            if (success)
            {
                ...
                if (plyr->_GetSkillLineCurrent(SKILL_FISHING, false) < maxskill)
                    plyr->_AdvanceSkillLine(SKILL_FISHING, float2int32(1.0f * sWorld.settings.getFloatRate(RATE_SKILLRATE)));
