...
void WorldSession::HandleWhoOpcode(WorldPacket& recv_data)
{
    ...
    while (itr != iend && sent_count < 49)   // WhoList should display 49 names not including your own
    {
        ...
        if (!sWorld.settings.server.showGmInWhoList && !HasGMPermissions())
        {
        ...
        // Team check
        if (!gm && plr->GetTeam() != team && !plr->GetSession()->HasGMPermissions() && !sWorld.settings.interfaction.isInterfactionMiscEnabled)
            continue;
...
void WorldSession::HandleLogoutRequestOpcode(WorldPacket& recv_data)
{
    ...
    if (pPlayer)
    {
        ...
        if (GetPermissionCount() == 0)
        {
            ...
            if (pPlayer->m_isResting || pPlayer->GetTaxiState() || sWorld.settings.optional.enableInstantLogoutForAccessType == 2)
            {
        ...
        if (GetPermissionCount() > 0)
        {
            if (pPlayer->m_isResting || pPlayer->GetTaxiState() || sWorld.settings.optional.enableInstantLogoutForAccessType > 0)
            {
                //Logout on NEXT sessionupdate to preserve processing of dead packets (all pending ones should be processed)
                SetLogoutTimer(1);
                return;
