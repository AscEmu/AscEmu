...
void WorldSession::handleWhoOpcode(WorldPacket& recvPacket)
{
    ...
    logger.debug("Received CMSG_WHO with %u zones and %u names", srlPacket.zone_count, srlPacket.name_count);
...
void WorldSession::handleTutorialFlag(WorldPacket& recvPacket)
{
    ...
    logger.debug("Received Tutorial flag: (%u).", srlPacket.flag);
}
...
void WorldSession::handlePlayedTimeOpcode(WorldPacket& recvPacket)
{
    ...
    logger.debug("Received CMSG_PLAYED_TIME: displayinui: %u", srlPacket.displayInUi);
    ...
    logger.debug("Sent SMSG_PLAYED_TIME total: %u level: %u", _player->m_playedtime[1], _player->m_playedtime[0]);
}
...
void WorldSession::handleSetActionButtonOpcode(WorldPacket& recvPacket)
{
    ...
    logger.debug("BUTTON: %u ACTION: %u TYPE: %u MISC: %u", srlPacket.button, srlPacket.action, srlPacket.type, srlPacket.misc);

    if (srlPacket.action == 0)
    {
        logger.debug("MISC: Remove action from button %u", srlPacket.button);
    ...
    else
    {
        ...
        {
            logger.debug("MISC: Added Macro %u into button %u", srlPacket.action, srlPacket.button);
            _player->setActionButton(srlPacket.button, srlPacket.action, srlPacket.type, srlPacket.misc);
        }
        else if (srlPacket.type == 128)
        {
            logger.debug("MISC: Added Item %u into button %u", srlPacket.action, srlPacket.button);
            _player->setActionButton(srlPacket.button, srlPacket.action, srlPacket.type, srlPacket.misc);
        }
        else if (srlPacket.type == 0)
        {
            logger.debug("MISC: Added Spell %u into button %u", srlPacket.action, srlPacket.button);
            _player->setActionButton(srlPacket.button, srlPacket.action, srlPacket.type, srlPacket.misc);
        }
    }
}
...
void WorldSession::handleRandomRollOpcode(WorldPacket& recvPacket)
{
    ...
    logger.debug("Received MSG_RANDOM_ROLL: %u (min), %u (max)", srlPacket.min, srlPacket.max);
...
void WorldSession::handleRealmSplitOpcode(WorldPacket& recvPacket)
{
    ...
    logger.debug("Received CMSG_REALM_SPLIT: %u (unk)", srlPacket.unknown);
...
void WorldSession::handleSetTaxiBenchmarkOpcode(WorldPacket& recvPacket)
{
    ...
    logger.debug("Received CMSG_SET_TAXI_BENCHMARK_MODE: %d (mode)", srlPacket.mode);
}
...
void WorldSession::handleGameobjReportUseOpCode(WorldPacket& recvPacket)
{
    ...
    logger.debug("Received CMSG_GAMEOBJ_REPORT_USE: %u (guid.low)", srlPacket.guid.getGuidLow());
...
void WorldSession::handleDungeonDifficultyOpcode(WorldPacket& recvPacket)
{
    ...
    logger.debug("Received MSG_SET_DUNGEON_DIFFICULTY: %d (difficulty)", srlPacket.difficulty);
...
void WorldSession::handleRaidDifficultyOpcode(WorldPacket& recvPacket)
{
    ...
    logger.debug("Received MSG_SET_RAID_DIFFICULTY: %d (difficulty)", srlPacket.difficulty);
...
void WorldSession::handleSetAutoLootPassOpcode(WorldPacket& recvPacket)
{
    ...
    logger.debug("Received CMSG_OPT_OUT_OF_LOOT: %u (turnedOn)", srlPacket.turnedOn);
...
void WorldSession::handleSetActionBarTogglesOpcode(WorldPacket& recvPacket)
{
    ...
    logger.debug("Received CMSG_SET_ACTIONBAR_TOGGLES: %d (actionbarId)", srlPacket.actionbarId);
...
void WorldSession::handleLootRollOpcode(WorldPacket& recvPacket)
{
    ...
    logger.debug("Received CMSG_LOOT_ROLL: %u (objectGuid) %u (slot) %d (choice)", srlPacket.objectGuid.getGuidLow(), srlPacket.slot, srlPacket.choice);
...
void WorldSession::handleOpenItemOpcode(WorldPacket& recvPacket)
{
    ...
    logger.debug("Received CMSG_OPEN_ITEM: %u (containerSlot), %u (slot)", srlPacket.containerSlot, srlPacket.slot);
...
void WorldSession::handleUpdateAccountData(WorldPacket& recvPacket)
{
    ...
    if (srlPacket.uiId > 8)
    {
        logger.failure("WARNING: Accountdata > 8 (%u) was requested to be updated by %s of account %u!",
    ...
    if (srlPacket.uiDecompressedSize > receivedPackedSize)
    {
        ...
        switch (ZlibResult)
        {
            case Z_OK:                  //0 no error decompression is OK
            {
                ...
                logger.debug("Successfully decompressed account data %d for %s, and updated storage array.",
            ...
            {
                delete[] data;
                logger.failure("Decompression of account data %u for %s FAILED.", srlPacket.uiId, _player->getName().c_str());
            } break;

            default:
            {
                delete[] data;
                logger.failure("Decompression gave a unknown error: %x, of account data %u for %s FAILED.",
...
void WorldSession::handleRequestAccountData(WorldPacket& recvPacket)
{
    ...
    logger.debug("Received CMSG_REQUEST_ACCOUNT_DATA id %u.", accountDataId);

    if (accountDataId > 8)
    {
        logger.debug("CMSG_REQUEST_ACCOUNT_DATA: Accountdata > 8 (%d) was requested by %s of account %u!", accountDataId, _player->getName().c_str(), this->GetAccountId());
        return;
    }
    ...
    else
    {
        ...
        if (accountDataEntry->sz > 200)
        {
            ...
            {
                logger.debug("CMSG_REQUEST_ACCOUNT_DATA: Error while compressing data");
                return;
            }
...
void WorldSession::handleBugOpcode(WorldPacket& recv_data)
{
    ...
    if (srlPacket.suggestion == 0)
        LOGGER.debug("Received CMSG_BUG [Bug Report]");
    else
        LOGGER.debug("Received CMSG_BUG [Suggestion]");
...
void WorldSession::handleBugOpcode(WorldPacket& recv_data)
{
    ...
    logger.debug("Received CMSG_BUG [Bug Report] lenght: %u message: %s", lenght, bugMessage.c_str());
...
void WorldSession::handleSuggestionOpcode(WorldPacket& recvPacket)
{
    ...
    logger.debug("Received CMSG_SUGGESTIONS [Suggestion] lenght: %u message: %s", lenght, suggestionMessage.c_str());
...
void WorldSession::handleLogDisconnectOpcode(WorldPacket& recvPacket)
{
    ...
    logger.debug("Player %s disconnected on %s - Reason %u", _player->getName().c_str(),
...
{
    logger.debug("Received CMSG_COMPLETE_CINEMATIC");
...
{
    CHECK_INWORLD_RETURN
    logger.debug("Received CMSG_NEXT_CINEMATIC_CAMERA");
...
{
    logger.debug("Received CMSG_READY_FOR_ACCOUNT_DATA_TIMES");
...
void WorldSession::handleLogoutCancelOpcode(WorldPacket& /*recvPacket*/)
{
    ...
    logger.debug("Received CMSG_LOGOUT_CANCEL");
    ...
    logger.debug("Sent SMSG_LOGOUT_CANCEL_ACK");
}
...
void WorldSession::handlePlayerLogoutOpcode(WorldPacket& /*recvPacket*/)
{
    ...
    logger.debug("Received CMSG_PLAYER_LOGOUT");
    if (!HasGMPermissions())
        SendNotification("You do not have permission to perform that function.");
    else
        LogoutPlayer(true);
...
void WorldSession::handleCorpseReclaimOpcode(WorldPacket& recvPacket)
{
    ...
    logger.debug("Received CMSG_RECLAIM_CORPSE");
...
void WorldSession::handleObjectUpdateFailedOpcode(WorldPacket& recvPacket)
{
    ...
    logger.failure("handleObjectUpdateFailedOpcode : Object update failed for playerguid %u", WoWGuid::getGuidLowPartFromUInt64(guid));
    ...
    for (uint32_t i = 0; i < count; ++i)
    {
        ...
        switch (type)
        {
            ...
            case DB2_REPLY_SPARSE:
                ...
                break;
            default:
                LOGGER.debug("Received unknown hotfix type %u", type);
                recvPacket.clear();
                break;
        }
    }
...
{
    logger.debug("Received CMSG_REQUEST_CEMETERY_LIST");
...
void WorldSession::handleBarberShopResult(WorldPacket& recvPacket)
{
    ...
    logger.debug("Received CMSG_ALTER_APPEARANCE");
...
void WorldSession::handleRepopRequestOpcode(WorldPacket& /*recvPacket*/)
{
    ...
    logger.debug("Received CMSG_REPOP_REQUEST");
...
void WorldSession::handleWhoIsOpcode(WorldPacket& recvPacket)
{
    ...
    logger.debug("Received WHOIS command from player %s for character %s", _player->getName().c_str(), srlPacket.characterName.c_str());
...
void WorldSession::handleGameObjectUse(WorldPacket& recvPacket)
{
    ...
    logger.debug("Received CMSG_GAMEOBJ_USE: %u (gobj guidLow)", srlPacket.guid.getGuidLowPart());
    ...
    switch (gameObject->getGoType())
    {
        ...
        case GAMEOBJECT_TYPE_BARBER_CHAIR:
            ...
            break;
        default:
            logger.debug("Received CMSG_GAMEOBJ_USE for unhandled type %u.", gameObject->getGoType());
            break;
    }
}
...
void WorldSession::handleInspectOpcode(WorldPacket& recvPacket)
{
    ...
    logger.debug("Received CMSG_INSPECT: %u (player guid)", static_cast<uint32_t>(srlPacket.guid));

    auto inspectedPlayer = _player->GetMapMgr()->GetPlayer(static_cast<uint32_t>(srlPacket.guid));
    if (inspectedPlayer == nullptr)
    {
        logger.debug("Error received CMSG_INSPECT for unknown player!");
        return;
    }
...
void WorldSession::readAddonInfoPacket(ByteBuffer &recvPacket)
{
    ...
    if (recvSize > 0xFFFFF)
    {
        logger.debug("recvSize %u too long", recvSize);
        return;
    }
    ...
    if (uncompress(unpackedInfo.contents(), &uSize, recvPacket.contents() + pos, static_cast<uLong>(recvPacket.size() - pos)) == Z_OK)
    {
        ...
        for (uint32_t i = 0; i < addonsCount; ++i)
        {
            ...
            logger.debug("AddOn: %s (CRC: 0x%x) - enabled: 0x%x - Unknown2: 0x%x", addonName.c_str(), crc, enabledState, unknown);
            ...
            if (savedAddon)
            {
                if (addon.crc != savedAddon->crc)
                    logger.debug("Addon: %s: modified (CRC: 0x%x) - accountID %d)", addon.name.c_str(), savedAddon->crc, GetAccountId());
                else
                    logger.debug("Addon: %s: validated (CRC: 0x%x) - accountID %d", addon.name.c_str(), savedAddon->crc, GetAccountId());
            }
            else
            {
                sAddonMgr.SaveAddon(addon);
                logger.debug("Addon: %s: unknown (CRC: 0x%x) - accountId %d (storing addon name and checksum to database)", addon.name.c_str(), addon.crc, GetAccountId());
            }
    ...
    {
        logger.failure("Decompression of addon section of CMSG_AUTH_SESSION failed.");
    }
}
...
void WorldSession::sendAddonInfo()
{
    ...
    for (auto itr : m_addonList)
    {
        ...
        if (crcpub)
        {
            ...
            {
                LOGGER.debug("AddOn: %s: CRC checksum mismatch: got 0x%x - expected 0x%x - sending pubkey to accountID %d",
...
{
    logger.debug("Received CMSG_UNREGISTER_ALL_ADDON_PREFIXES");
...
{
    logger.debug("Received CMSG_REPORT");
    ...
    switch (spam_type)
    {
        case 0:
        {
            ...
            logger.debug("Received REPORT SPAM: type %u, guid %u, unk1 %u, unk2 %u, unk3 %u", spam_type, WoWGuid::getGuidLowPartFromUInt64(spammer_guid), unk1, unk2, unk3);
        ...
        case 1:
        {
            ...
            logger.debug("Received REPORT SPAM: type %u, guid %u, unk1 %u, unk2 %u, unk3 %u, unk4 %u, message %s", spam_type, WoWGuid::getGuidLowPartFromUInt64(spammer_guid), unk1, unk2, unk3, unk4, description.c_str());
...
{
    logger.debug("Received CMSG_REPORT_PLAYER %u", static_cast<uint32_t>(recvPacket.size()));
    ...
    switch (unk3)
    {
        case 0:     // chat spamming
            logger.debug("Chat spamming report for guid: %u received.", unk4 + 1);
            break;
        case 2:     // cheat
            recvPacket >> message;
            logger.debug("Cheat report for guid: %u received. Message %s", unk4 + 1, message.c_str());
            break;
        case 6:     // char name
            recvPacket >> message;
            logger.debug("char name report for guid: %u received. Message %s", unk4 + 1, message.c_str());
            break;
        case 12:     // guild name
            recvPacket >> message;
            logger.debug("guild name report for guid: %u received. Message %s", unk4 + 1, message.c_str());
            break;
        case 18:     // arena team name
            recvPacket >> message;
            logger.debug("arena team name report for guid: %u received. Message %s", unk4 + 1, message.c_str());
            break;
        case 20:     // chat language
            logger.debug("Chat language report for guid: %u received.", unk4 + 1);
            break;
        default:
            logger.debug("type is %u", unk3);
            break;
    }
}
...
void WorldSession::HandleMirrorImageOpcode(WorldPacket& recv_data)
{
    ...
    logger.debug("Received CMG_GET_MIRRORIMAGE_DATA");
    ...
    logger.debug("Sent SMSG_MIRRORIMAGE_DATA");
}
