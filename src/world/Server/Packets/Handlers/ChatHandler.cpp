...
void WorldSession::HandleMessagechatOpcode(WorldPacket& recv_data)
{
    ...
    // Flood protection
    if (lang != -1 && !GetPermissionCount() && sWorld.floodProtectionSettings.linesBeforeProtection != 0)
    {
        /* flood detection, wheeee! */
        if (UNIXTIME >= floodTime)
        {
            floodLines = 0;
            floodTime = UNIXTIME + sWorld.floodProtectionSettings.secondsBeforeProtectionReset;
        }

        if ((++floodLines) > sWorld.floodProtectionSettings.linesBeforeProtection)
        {
            if (sWorld.floodProtectionSettings.enableSendFloodProtectionMessage)
    ...
    switch (type)
    {
        case CHAT_MSG_EMOTE:
        {
            if (sWorld.interfactionSettings.isInterfactionChatEnabled && lang > 0)
                lang = 0;
            ...
            else if (lang == 0 && sWorld.interfactionSettings.isInterfactionChatEnabled)
            ...
            GetPlayer()->SendMessageToSet(data, true, !sWorld.interfactionSettings.isInterfactionChatEnabled);
        ...
        case CHAT_MSG_SAY:
        {
            ...
            else
            {
                ...
                if (lang == 0 && !CanUseCommand('c') && !sWorld.interfactionSettings.isInterfactionChatEnabled)
                    return;
        ...
        case CHAT_MSG_YELL:
        {
            ...
            if (lang == 0 && sWorld.interfactionSettings.isInterfactionChatEnabled)
        ...
        case CHAT_MSG_WHISPER:
        {
            ...
            if (_player->GetTeamInitial() != playercache->GetUInt32Value(CACHE_PLAYER_INITIALTEAM) && !sWorld.interfactionSettings.isInterfactionChatEnabled && !playercache->HasFlag(CACHE_PLAYER_FLAGS, PLAYER_FLAG_GM) && !_player->HasFlag(PLAYER_FLAGS, PLAYER_FLAG_GM))
            {
...
void WorldSession::HandleTextEmoteOpcode(WorldPacket& recv_data)
{
    ...
    if (!GetPermissionCount() && sWorld.floodProtectionSettings.linesBeforeProtection)
    {
        /* flood detection, wheeee! */
        if (UNIXTIME >= floodTime)
        {
