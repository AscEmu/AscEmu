...
void WorldSession::HandleMoveTeleportAckOpcode(WorldPacket& recv_data)
{
    ...
    {
        if (sWorld.settings.antiHack.isTeleportHackCheckEnabled && !(HasGMPermissions() && sWorld.settings.antiHack.isAntiHackCheckDisabledForGm) && _player->GetPlayerStatus() != TRANSFER_PENDING)
        {
        ...
        if (sWorld.settings.antiHack.isTeleportHackCheckEnabled && !(HasGMPermissions() && sWorld.settings.antiHack.isAntiHackCheckDisabledForGm) && _player->m_position.Distance2DSq(_player->m_sentTeleportPosition) > 625.0f)	/* 25.0f*25.0f */
        {
            /* cheating.... :(*/
...
void _HandleBreathing(MovementInfo & movement_info, Player* _player, WorldSession* pSession)
{
    ...
    if (!sWorld.settings.server.enableBreathing || _player->FlyCheat || _player->m_bUnlimitedBreath || !_player->isAlive() || _player->GodModeCheat)
    {
...
void WorldSession::HandleMovementOpcodes(WorldPacket& recv_data)
{
    ...
    if (!(HasGMPermissions() && sWorld.settings.antiHack.isAntiHackCheckDisabledForGm) && !_player->GetCharmedUnitGUID())
    {
        /************************************************************************/
        /* Anti-Teleport                                                        */
        /************************************************************************/
        if (sWorld.settings.antiHack.isTeleportHackCheckEnabled && _player->m_position.Distance2DSq(movement_info.position.x, movement_info.position.y) > 3025.0f
    ...
    //update the detector
    if (sWorld.settings.antiHack.isSpeedHackCkeckEnabled && !_player->GetTaxiState() && _player->obj_movement_info.transporter_info.guid == 0 && !_player->GetSession()->GetPermissionCount())
    {
