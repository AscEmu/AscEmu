...
void WorldSession::handleSaveGuildEmblem(WorldPacket& recvPacket)
{
    ...
    LOGGER.debug("MSG_SAVE_GUILD_EMBLEM %s: vendorGuid: %u style: %u, color: %u, borderStyle: %u, borderColor: %u, backgroundColor: %u",
...
void WorldSession::handleGuildLeader(WorldPacket& recvPacket)
{
    ...
    const auto targetPlayerInfo = sObjectMgr.GetPlayerInfoByName(srlPacket.name.c_str());
...
void WorldSession::handleGuildSetPublicNote(WorldPacket& recvPacket)
{
    ...
    const auto targetPlayerInfo = sObjectMgr.GetPlayerInfoByName(srlPacket.targetName.c_str());
    if (targetPlayerInfo == nullptr)
        return;
...
void WorldSession::handleCharterTurnInCharter(WorldPacket& recvPacket)
{
    ...
    else
    {
        ...
        for (uint32_t i = 0; i < charter->SignatureCount; ++i)
            if (PlayerInfo* info = sObjectMgr.GetPlayerInfo(charter->Signatures[i]))
...
void WorldSession::handleGuildAssignRankOpcode(WorldPacket& recvPacket)
{
    ...
    LOGGER.debug("CMSG_GUILD_ASSIGN_MEMBER_RANK %s: Target: %u Rank: %u, Issuer: %u",
...
void WorldSession::handleGuildQueryRanksOpcode(WorldPacket& recvPacket)
{
    ...
    LOGGER.debug("CMSG_GUILD_QUERY_RANKS %s: Guild: %u", _player->getName().c_str(), WoWGuid::getGuidLowPartFromUInt64(guildGuid));
...
void WorldSession::handleGuildQueryXPOpcode(WorldPacket& recvPacket)
{
    ...
    LOGGER.debug("CMSG_QUERY_GUILD_XP %s: guildId: %u", _player->getName().c_str(), guildId);
