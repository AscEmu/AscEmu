...
void Player::Reputation_OnKilledUnit(Unit* pUnit, bool InnerLoop)
{
    ...
    if (modifier != nullptr)
    {
        ...
        for (std::vector<ReputationMod>::iterator itr = modifier->mods.begin(); itr != modifier->mods.end(); ++itr)
        {
            ...
            ModStanding(itr->faction[team], float2int32(itr->value * sWorld.settings.getFloatRate(RATE_KILLREPUTATION)));
        }
    }
    else
    {
        ...
        int32 change = int32(-5.0f * sWorld.settings.getFloatRate(RATE_KILLREPUTATION));
