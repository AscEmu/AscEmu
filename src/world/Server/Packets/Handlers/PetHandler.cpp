...
void WorldSession::handlePetAction(WorldPacket& recvPacket)
{
    ...
    for (auto itr = summons.begin(); itr != summons.end();)
    {
        ...
        switch (srlPacket.action)
        {
            ...
            break;
            default:
                logger.debug("WARNING: Unknown pet action received. Action = %u, Misc = %u", srlPacket.action, srlPacket.misc);
            break;
        }
...
void WorldSession::handleUnstablePet(WorldPacket& recvPacket)
{
    ...
    if (playerPet == nullptr)
    {
        logger.failure("PET SYSTEM: Player " I64FMT " tried to unstable non-existent pet %u", _player->getGuid(), srlPacket.petNumber);
        return;
    }
...
void WorldSession::handleDismissCritter(WorldPacket& recvPacket)
{
    ...
    {
        logger.failure("Player %u sent dismiss companion packet, but player has no companion", _player->getGuidLow());
        return;
    }

    if (_player->getCritterGuid() != srlPacket.guid.getRawGuid())
    {
        logger.failure("Player %u sent dismiss companion packet, but it doesn't match player's companion", _player->getGuidLow());
        return;
    }
