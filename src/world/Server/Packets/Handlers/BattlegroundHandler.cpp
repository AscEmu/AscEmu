...
void WorldSession::handleInspectHonorStatsOpcode(WorldPacket& recvPacket)
{
    ...
    logger.debug("Received MSG_INSPECT_HONOR_STATS: %u (guidLow)", srlPacket.guid.getGuidLow());
...
void WorldSession::handleArenaJoinOpcode(WorldPacket& recvPacket)
{
    ...
    switch (srlPacket.category)
    {
        ...
        case 2:
            ...
            break;
        default:
            logger.debug("Received CMSG_BATTLEMASTER_JOIN_ARENA: with invalid category (%u)", srlPacket.category);
            battlegroundType = 0;
            break;
    }
...
void WorldSession::handleBattlefieldListOpcode(WorldPacket& recvPacket)
{
    ...
    logger.debug("Received CMSG_BATTLEFIELD_LIST: %u (bgType), %u (fromType)", srlPacket.bgType, srlPacket.fromType);
...
void WorldSession::handleBattleMasterHelloOpcode(WorldPacket& recvPacket)
{
    ...
    logger.debug("Received CMSG_BATTLEMASTER_HELLO: %u (guidLowPart)", srlPacket.guid.getGuidLowPart());
...
void WorldSession::handleAreaSpiritHealerQueueOpcode(WorldPacket& recvPacket)
{
    ...
    logger.debug("Received CMSG_AREA_SPIRIT_HEALER_QUEUE: %u (guidLowPart)", srlPacket.guid.getGuidLowPart());
...
void WorldSession::handleRequestRatedBgInfoOpcode(WorldPacket & recvPacket)
{
    ...
    logger.debug("Received CMSG_REQUEST_RATED_BG_INFO received with unk_type = %u", srlPacket.type);
...
{
    logger.debug("Received CMSG_REQUEST_RATED_BG_STATS received");
