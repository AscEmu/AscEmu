...
void WorldSession::nothingToHandle(WorldPacket& recv_data)
{
    if (!recv_data.isEmpty())
    {
        sLogger.debugOpcode("Opcode {} [{}] (0x{:04X}) received. Apply nothingToHandle handler but size is {}!",
            sOpcodeTables.getNameForOpcode(recv_data.getOpcode(), getClientProtocol()), WoW::getNameForVersionId(getClientProtocol().versionId()), recv_data.getOpcode(), recv_data.size());
    }
}
