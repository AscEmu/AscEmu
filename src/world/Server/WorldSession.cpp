...
uint8 WorldSession::Update(uint32 InstanceID)
{
    ...
    while ((packet = _recvQueue.pop()) != nullptr)
    {
        if (packet != nullptr)
        {
            ...
            {
                sLogger.debugFlag(AscEmu::Logging::LF_OPCODE, "[Session] Received out of range packet with opcode 0x{:04X}", packet->GetOpcode());
            }
            else
            {
...
void WorldSession::loadHandlers() // Classic
{
    ...
    // Unsorted
    //WorldPacketHandlers[CMSG_TIME_SYNC_RESPONSE].handler = &WorldSession::HandleTimeSyncResp; // MiscHandler.cpp
...
void WorldSession::loadHandlers() // WotLK
{
    ...
    // Unsorted
    //WorldPacketHandlers[CMSG_TIME_SYNC_RESPONSE].handler = &WorldSession::HandleTimeSyncResp;
    WorldPacketHandlers[CMSG_READY_FOR_ACCOUNT_DATA_TIMES].handler = &WorldSession::handleReadyForAccountDataTimes;
    WorldPacketHandlers[CMSG_READY_FOR_ACCOUNT_DATA_TIMES].status = STATUS_AUTHED;
