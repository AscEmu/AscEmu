...
void ScriptMgr::register_spell_script(uint32_t spellId, SpellScript* ss)
{
    ...
    {
        LOGGER.failure("ScriptMgr tried to register a script for spell id %u but spell does not exist!", spellId);
        return;
    }

    if (_spellscripts.find(spellId) != _spellscripts.end())
    {
        LOGGER.debug("ScriptMgr tried to register a script for spell id %u but this spell has already one.", spellId);
        return;
    }
...
{
    LOGGER.info("ScriptMgr : Loading External Script Libraries...");
    ...
    for (const auto content : directoryContentMap)
    {
        ...
        if (!dynLib->Load())
        {
            ...
            LOGGER.failure(loadMessageStream.str().c_str());
            delete dynLib;
            continue;
        }
        ...
        LOGGER.info(loadMessageStream.str().c_str());
    ...
    if (dllCount == 0)
    {
        LOGGER.failure("No external scripts found! Server will continue to function with limited functionality.");
    }
    else
    {
        LOGGER.info("ScriptMgr : Loaded %u external libraries.", dllCount);
        LOGGER.info("ScriptMgr : Loading optional scripting engine(s)...");
        ...
        LOGGER.info("ScriptMgr : Done loading scripting engine(s)...");
    }
}
...
void ScriptMgr::DumpUnimplementedSpells()
{
    ...
    LOGGER.info("Dumping IDs for spells with unimplemented dummy/script effect(s)");
    uint32 count = 0;
    ...
    LOGGER.info("Dumped %u IDs.", count);

    LOGGER.info("Dumping IDs for spells with unimplemented dummy aura effect.");
...
void ScriptMgr::register_creature_script(uint32 entry, exp_create_creature_ai callback)
{
    ...
    if (_creatures.find(entry) != _creatures.end())
        LOGGER.debug("ScriptMgr tried to register a script for Creature ID: %u but this creature has already one!", entry);
...
void ScriptMgr::register_gameobject_script(uint32 entry, exp_create_gameobject_ai callback)
{
    if (_gameobjects.find(entry) != _gameobjects.end())
        LOGGER.debug("ScriptMgr tried to register a script for GameObject ID: %u but this go has already one.", entry);
...
void ScriptMgr::register_dummy_aura(uint32 entry, exp_handle_dummy_aura callback)
{
    ...
    {
        LOGGER.debug("ScriptMgr tried to register a script for Aura ID: %u but this aura has already one.", entry);
    }

    SpellInfo const* sp = sSpellMgr.getSpellInfo(entry);
    if (sp == NULL)
    {
        LOGGER.debug("ScriptMgr tried to register a dummy aura handler for invalid Spell ID: %u.", entry);
        return;
    }

    if (!sp->hasEffectApplyAuraName(SPELL_AURA_DUMMY) && !sp->hasEffectApplyAuraName(SPELL_AURA_PERIODIC_TRIGGER_DUMMY))
        LOGGER.debug("ScriptMgr registered a dummy aura handler for Spell ID: %u (%s), but spell has no dummy aura!", entry, sp->getName().c_str());
...
void ScriptMgr::register_dummy_spell(uint32 entry, exp_handle_dummy_spell callback)
{
    ...
    {
        LOGGER.debug("ScriptMgr tried to register a script for Spell ID: %u but this spell has already one", entry);
        return;
    }
    ...
    if (sp == NULL)
    {
        LOGGER.debug("ScriptMgr tried to register a dummy handler for invalid Spell ID: %u.", entry);
        return;
    }

    if (!sp->hasEffect(SPELL_EFFECT_DUMMY) && !sp->hasEffect(SPELL_EFFECT_SCRIPT_EFFECT) && !sp->hasEffect(SPELL_EFFECT_SEND_EVENT))
        LOGGER.debug("ScriptMgr registered a dummy handler for Spell ID: %u (%s), but spell has no dummy/script/send event effect!", entry, sp->getName().c_str());
...
void ScriptMgr::register_quest_script(uint32 entry, QuestScript* qs)
{
    ...
    if (q != nullptr)
    {
        if (q->pQuestScript != NULL)
            LOGGER.debug("ScriptMgr tried to register a script for Quest ID: %u but this quest has already one.", entry);
...
void ScriptMgr::register_event_script(uint32 entry, EventScript* es)
{
    ...
    if (gameEvent != nullptr)
    {
        ...
        {
            LOGGER.debug("ScriptMgr tried to register a script for Event ID: %u but this event has already one.", entry);
            return;
        }
...
void ScriptMgr::register_instance_script(uint32 pMapId, exp_create_instance_ai pCallback)
{
    if (mInstances.find(pMapId) != mInstances.end())
        LOGGER.debug("ScriptMgr tried to register a script for Instance ID: %u but this instance already has one.", pMapId);
...
void ScriptMgr::register_script_effect(uint32 entry, exp_handle_script_effect callback)
{
    ...
    {
        LOGGER.debug("ScriptMgr tried to register more than 1 script effect handlers for Spell %u", entry);
        return;
    }
    ...
    if (sp == NULL)
    {
        LOGGER.debug("ScriptMgr tried to register a script effect handler for invalid Spell %u.", entry);
        return;
    }

    if (!sp->hasEffect(SPELL_EFFECT_SCRIPT_EFFECT) && !sp->hasEffect(SPELL_EFFECT_SEND_EVENT))
        LOGGER.debug("ScriptMgr registered a script effect handler for Spell ID: %u (%s), but spell has no scripted effect!", entry, sp->getName().c_str());
...
void InstanceScript::addData(uint32_t data, uint32_t state /*= NotStarted*/)
{
    ...
    if (Iter == mInstanceData.end())
        ...
        LOGGER.debug("InstanceScript::addData - tried to set state for entry %u. The entry is already available with a state!", data);
}
...
void InstanceScript::setData(uint32_t data, uint32_t state)
{
    ...
    if (Iter != mInstanceData.end())
    {
        ...
        LOGGER.debug("InstanceScript::setData - tried to set state for entry %u on map %u. The entry is not defined in table instance_bosses or manually to handle states!", data, mInstance->GetMapId());
}
...
void InstanceScript::generateBossDataState()
{
    ...
    if (encounters != nullptr)
    {
        ...
        for (DungeonEncounterList::const_iterator itr = encounters->begin(); itr != encounters->end(); ++itr)
        {
            ...
            if (encounter->creditType == ENCOUNTER_CREDIT_KILL_CREATURE)
            {
                ...
                if (creature == nullptr)
                    LOGGER.failure("Your instance_encounters table includes invalid data for boss entry %u!", encounter->creditEntry);
                else
    ...
    LOGGER.debug("InstanceScript::generateBossDataState() - Boss State generated for map %u.", mInstance->GetMapId());
}
...
Creature* InstanceScript::spawnCreature(uint32_t entry, float posX, float posY, float posZ, float posO, uint32_t factionId /* = 0*/)
{
    ...
    {
        LOGGER.failure("tried to create a invalid creature with entry %u!", entry);
        return nullptr;
    }
