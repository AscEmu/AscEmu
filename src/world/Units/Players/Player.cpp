    ...
    if (const auto raceEntry = sChrRacesStore.LookupEntry(race))
    {
        switch (gender)
        {
            ...
            case GENDER_FEMALE:
                ...
                break;
            default:
                LOGGER.failure("Gender %u is not valid for Player charecters!", gender);
        }
    }
    else
    {
        LOGGER.failure("Race %u is not supported by this AEVersion (%u)", race, getAEVersion());
    }
}
...
void Player::learnTalent(uint32_t talentId, uint32_t talentRank)
{
    ...
    if (spellId == 0)
    {
        LOGGER.info("Player::learnTalent: Player tried to learn talent %u (rank %u) but talent's spell id is 0.", talentId, talentRank);
        return;
    }
