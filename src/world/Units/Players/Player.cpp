...
void Player::LearnTalent(uint32 talentid, uint32 rank, bool isPreviewed)
{
    ...
    else
    {
        ...
        for (uint8 i = rank; i < 5; ++i)
        {
            ...
            {
                sLog.outDebug("LearnTalent: Has rank %u higher already higher or equal.Spell id %u", i, talent_info->RankID[i]);
                 //return; // cheater
            }
        }
...
void Player::AcceptQuest(uint64 guid, uint32 quest_id)
{
    ...
    if (guidtype == HIGHGUID_TYPE_UNIT)
    {
        ...
        if (quest_giver)
            ...
            return;
        hasquest = quest_giver->HasQuest(quest_id, QUESTGIVER_QUEST_START);
