            ...
            else
            {
                ...
                switch (r)
                {
                    ...
                    case 5:
                    {
                        ...
                        //LogDefault("DEBUG: Critical Strike! Full_damage: %u" , dmg.full_damage);
                        if (ability != nullptr)
                        {
                        ...
                        //LogDefault("DEBUG: After CritMeleeDamageTakenPctMod: %u" , dmg.full_damage);
                        if (IsPlayer())
                        {
                            if (weapon_damage_type != RANGED)
                            {
                            ...
                            //LogDefault("DEBUG: After IncreaseCricticalByTypePCT: %u" , dmg.full_damage);
                        }
                        ...
                        if (pVictim->IsPlayer())
                        {
                            ...
                            //LogDefault("DEBUG: After Resilience check: %u" , dmg.full_damage);
                        }
...
{
    //LogDefault("Unit::MechanicImmunityMassDispel called, mechanic: %u" , MechanicType);
    ...
    for (uint32 x = (HostileOnly ? MAX_NEGATIVE_AURAS_EXTEDED_START : MAX_POSITIVE_AURAS_EXTEDED_START); x < MAX_REMOVABLE_AURAS_END; x++)    // If HostileOnly = 1, then we use aura slots 40-56 (hostile). Otherwise, we use 0-56 (all)
    {
        ...
        if (m_auras[x])
        {
            ...
            {
                //LogDefault("Removed aura. [AuraSlot %u, SpellId %u]" , x , m_auras[x]->GetSpellId());
