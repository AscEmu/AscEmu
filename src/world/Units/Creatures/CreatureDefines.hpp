...
struct CreatureProperties
{
    ...
    uint8_t GetGenderAndCreateRandomDisplayID(uint32_t* des) const
    {
        uint32_t models[] = { Male_DisplayID, Male_DisplayID2, Female_DisplayID, Female_DisplayID2 };
        ...
        while (true)
        {
            uint32_t res = Util::getRandomUInt(3);
            if (models[res])
            {
                *des = models[res];
                return res < 2 ? 0 : 1;
