...
void Creature::OnRemoveCorpse()
{
    ...
    if (IsInWorld() && (int32)m_mapMgr->GetInstanceID() == m_instanceId)
    {
        ...
        LOGGER.info("Removing corpse of " I64FMT "...", getGuid());
...
void Creature::OnRespawn(MapMgr* m)
{
    ...
    LOGGER.info("Respawning " I64FMT "...", getGuid());
...
bool Creature::Load(MySQLStructure::CreatureSpawn* spawn, uint8 mode, MySQLStructure::MapInfo const* info)
{
    ...
    {
        LOGGER.failure("MinHealth is bigger than MaxHealt! Using MaxHealth value. You should fix this in creature_proto table for entry: %u!", creature_properties->Id);
...
void Creature::OnPushToWorld()
{
    if (creature_properties == nullptr)
    {
        LOGGER.failure("Something tried to push Creature with entry %u with invalid creature_properties!", getEntry());
        return;
    }
...
void Creature::addVehicleComponent(uint32 creature_entry, uint32 vehicleid)
{
    ...
    {
        LOGGER.failure("Creature %u (%s) with GUID %u already has a vehicle component.", creature_properties->Id, creature_properties->Name.c_str(), GetUIdFromGUID());
        return;
    }
