...
void Pet::SetNameForEntry(uint32_t entry)
{
    switch (entry)
    {
        case PET_IMP:
...
bool Pet::CreateAsSummon(uint32_t entry, CreatureProperties const* ci, Creature* created_from_creature, Player* owner, SpellInfo const* created_by_spell, uint32_t type, uint32_t expiretime, LocationVector* Vec, bool dismiss_old_pet)
{
...
Pet::Pet(uint64_t guid) : Creature(guid), m_Owner(nullptr)
{
    Summon = false;
    memset(ActionBar, 0, sizeof(uint32_t) * 10);
...
Pet::~Pet()
{
    for (std::map<uint32_t, AI_Spell*>::iterator itr = m_AISpellStore.begin(); itr != m_AISpellStore.end(); ++itr)
        delete itr->second;
...
void Pet::Update(unsigned long time_passed)
{
    ...
    if (!Summon && !bExpires && isAlive())
    {
        ...
        {
            int32_t burn = 1042; // Based on WoWWiki pet looses 50 happiness over 6 min => 1042 every 7.5 s
...
void Pet::BuildPetSpellList(WorldPacket& data)
{
    ...
    data << uint32_t(0);
    data << uint8_t(GetPetState());       // 0x0 = passive, 0x1 = defensive, 0x2 = aggressive
...
void Pet::LoadFromDB(Player* owner, PlayerPet* pi)
{
    ...
    setPetTalentPoints(static_cast<uint8_t>(mPi->talentpoints));
...
void Pet::InitializeMe(bool first)
{
    ...
    if (Summon)         // Summons - always
    {
        ...
        if (getEntry() == WATER_ELEMENTAL || getEntry() == WATER_ELEMENTAL_NEW)
        {
            ...
            ModDamageDone[SCHOOL_FROST] = (uint32_t)parentfrost;
        }
...
void Pet::UpdatePetInfo(bool bSetToOffline)
{
    ...
    for (uint8_t i = 0; i < 10; ++i)
    {
        if (ActionBar[i] & 0x4000000)
            ...
            ss << ActionBar[i] << " " << uint32_t(GetSpellState(ActionBar[i]));
        else
            ss << "0 0";
...
void Pet::Despawn(uint32_t delay, uint32_t /*respawntime*/)
{
...
void Pet::DelayedRemove(bool bTime, bool dismiss, uint32_t delay)
{
    if (ScheduledForDeletion)
        return;
    ...
    if (bTime)
    {
        ...
        sEventMgr.AddEvent(this, &Pet::DelayedRemove, true, dismiss, uint32_t(0), EVENT_PET_DELAYED_REMOVE, delay, 1, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
}
...
uint32_t Pet::GetNextLevelXP(uint32_t level)
{
    // Pets need only 5% of xp to level up compared to players
    uint32_t nextLvlXP = 0;
...
void Pet::UpdateSpellList(bool showLearnSpells)
{
    ...
    {
        std::map<uint32_t, std::set<uint32_t>>::iterator it1 = m_Owner->SummonSpells.find(getEntry()); // Get spells from the owner
...
uint16_t Pet::GetSpellState(SpellInfo const* sp)
{
    PetSpellMap::iterator itr = mSpells.find(sp);
    if (itr == mSpells.end())
...
void Pet::ApplySummonLevelAbilities()
{
    ...
    // Calculate values
    BaseStats[STAT_STRENGTH] = (uint32_t)(pet_str);
    BaseStats[STAT_AGILITY] = (uint32_t)(pet_agi);
    BaseStats[STAT_STAMINA] = (uint32_t)(pet_sta);
    BaseStats[STAT_INTELLECT] = (uint32_t)(pet_int);
    BaseStats[STAT_SPIRIT] = (uint32_t)(pet_spr);
    ...
    BaseResistance[0] = (uint32_t)(pet_arm);
    CalcResistance(0);
...
void Pet::ApplyPetLevelAbilities()
{
    ...
    static uint32_t family_aura[47] = { 0   /*0*/,
...
{
    uint32_t days = (uint32_t)(UNIXTIME - reset_time) / 60 * 60 * 24;
...
void Pet::DealDamage(Unit* pVictim, uint32_t damage, uint32_t /*targetEvent*/, uint32_t /*unitEvent*/, uint32_t spellId, bool no_remove_auras)
{
        ...
        if (pVictim->isPlayer())
        {
            ...
            if (setAurastateFlag)
            {
                ...
                if (!sEventMgr.HasEvent(m_Owner, EVENT_LASTKILLWITHHONOR_FLAG_EXPIRE))
                    sEventMgr.AddEvent(static_cast< Unit* >(m_Owner), &Unit::removeAuraStateAndAuras, AURASTATE_FLAG_LASTKILLWITHHONOR, EVENT_LASTKILLWITHHONOR_FLAG_EXPIRE, 20000, 1, 0);
                else
...
void Pet::TakeDamage(Unit* pAttacker, uint32_t damage, uint32_t spellid, bool no_remove_auras)
{
    if (!no_remove_auras)
    {
