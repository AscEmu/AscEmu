...
void Pet::ApplySummonLevelAbilities()
{
    ...
    if (stat_index < 0)
    {
        LOGGER.failure("PETSTAT: No stat index found for entry %u, `%s`! Using 5 as a default.", getEntry(), GetCreatureProperties()->Name.c_str());
        stat_index = 5;
    }
    ...
    if (health == 0)
    {
        LOGGER.failure("Pet with entry %u has 0 health !!", getEntry());
        health = 100;
    }
...
void Pet::ApplyPetLevelAbilities()
{
    ...
    {
        LOGGER.failure("No abilities for level %u in table pet_level_abilities! Auto apply abilities of level 80!", level);
    ...
    //Family Aura
    if (pet_family > 46)
        LOGGER.failure("PETSTAT: Creature family %i [%s] has missing data.", pet_family, myFamily->name);
...
AI_Spell* Pet::HandleAutoCastEvent()
{
    ...
    for (std::list<AI_Spell*>::iterator itr2 = m_autoCastSpells[AUTOCAST_EVENT_ATTACK].begin(); itr2 != m_autoCastSpells[AUTOCAST_EVENT_ATTACK].end();)
    {
        ...
        {
            LOGGER.failure("Bad AI_Spell detected in AutoCastEvent!");
...
void Pet::HandleAutoCastEvent(AutoCastEvents Type)
{
    ...
    for (itr = m_autoCastSpells[Type].begin(); itr != m_autoCastSpells[Type].end();)
    {
        ...
        if (sp->spell == NULL)
        {
            LOGGER.failure("Found corrupted spell at m_autoCastSpells, skipping");
            continue;
        }
        else if (sp->autocast_type != static_cast<uint32>(Type))
        {
            LOGGER.failure("Found corrupted spell (%lu) at m_autoCastSpells, skipping", sp->entryId);
            continue;
        }
