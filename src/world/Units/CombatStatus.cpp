    ...
    namespace World
    {
        namespace Units
        {
            bool CombatStatus::isReallyInCombat() const
            {
                if (m_unit->IsPlayer())
                {
                    MapMgr* map_mgr = m_unit->GetMapMgr();
            ...
            {
                bool currentStatus = isReallyInCombat();
            ...
            void CombatStatus::clearAttackTargets()
            {
                ...
                {
                    Unit* target = map_mgr->GetUnit(targetGuid);
                    if (target == nullptr)
                    {
            ...
            void CombatStatus::removeAllHealersAndHealTargets()
            {
                ...
                if (map_mgr != nullptr)
                {
                    ...
                    {
                        Player* player = map_mgr->GetPlayer(healTargetGuid);
                        if (player != nullptr)
                        {
                    ...
                    for (auto healerGuid : m_healers)
                    {
                        Player* player = map_mgr->GetPlayer(healerGuid);
                        if (player != nullptr)
                        {
            ...
            void CombatStatus::removeAllAttackersAndAttackTargets()
            {
                ...
                if (map_mgr != nullptr)
                {
                    ...
                    {
                        Unit* attackTarget = map_mgr->GetUnit(attackTargetGuid);
                    ...
                    {
                        Unit* attacker = map_mgr->GetUnit(attackerGuid);
                        if (attacker != nullptr)
                        {
            ...
            void CombatStatus::clearPrimaryAttackTarget()
            {
                ...
                {
                    Unit* target = map_mgr->GetUnit(m_primaryAttackTarget);
                    if (target != nullptr)
                    {
            ...
            void CombatStatus::addAttacker(Unit* attacker)
            {
                ...
                uint64_t guid = attacker->GetGUID();
            ...
            {
                return std::find(begin(m_attackers), std::end(m_attackers), guid) != std::end(m_attackers);
            }
            ...
            void CombatStatus::onDamageDealt(Unit* target)
            {
                ...
                auto attackTarget = std::find(begin(m_attackTargets), end(m_attackTargets), target->GetGUID());
                if (attackTarget == std::end(m_attackTargets))
                {
