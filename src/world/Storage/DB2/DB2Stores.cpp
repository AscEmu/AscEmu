...
{
    logger.failure("LoadDB2_assert_print : Size of '%s' setted by format string (%u) not equal size of C++ structure (%u).", filename.c_str(), fsize, rsize);
...
void LoadDB2Stores()
{
    ...
    {
        logger.failure("LoadDB2Stores : Incorrect DataDir value in world.conf or ALL required *.db2 files (%d) not found", DB2_Count);
        exit(1);
    }
    else if (!bad_db2_files.empty())
    {
        ...
        logger.failure("LoadDB2Stores : Some required *.db2 files (%u from %d) not found or not compatible:%s", (uint32)bad_db2_files.size(), DB2_Count, str.c_str());
        exit(1);
    }

    if (!sItemStore.LookupEntry(83086) || !sItemExtendedCostStore.LookupEntry(3872))
    {
        logger.failure("LoadDB2Stores : Please extract correct db2 files from build 15595");
        exit(1);
    }

    logger.info("LoadDB2Stores : Initialized %u db2 stores", DB2_Count);
}
