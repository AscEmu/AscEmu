...
void MySQLDataStore::loadItemPagesTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `item_pages` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `item_pages` has %u columns", itempages_result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u pages from `item_pages` table in %u ms!", itempages_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadItemPropertiesTable()
{
    ...
    for (tableiterator = ItemPropertiesTables.begin(); tableiterator != ItemPropertiesTables.end(); ++tableiterator)
    {
        ...
        {
            logger.info("MySQLDataLoads : Table `%s` is empty!", table_name.c_str());
            return;
        }
        ...
        {
            logger.failure("Additional item_properties table `%s` has %u columns, but needs %u columns! Skipped!", table_name.c_str(), item_result->GetFieldCount(), basic_field_count);
            continue;
        }

        logger.info("MySQLDataLoads : Table `%s` has %u columns", table_name.c_str(), item_result->GetFieldCount());
        ...
        do
        {
            ...
            if (page_id != 0)
            {
                ...
                {
                    logger.failure("Table `%s` entry: %u includes invalid pageId %u! pageId is set to 0.", table_name.c_str(), entry, page_id);
    ...
    logger.info("MySQLDataLoads : Loaded %u item_properties in %u ms!", item_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
uint32_t const MySQLDataStore::getItemDisplayIdForEntry(uint32_t entry)
{
    if (entry != 0)
    {
        ...
        if (dbcDisplay != 0)
        {
            LOGGER.debug("Item entry %u not available in item_properties or has an invalid displayId! Using dbcDisplayId %u!", entry, dbcDisplay);
            return dbcDisplay;
        }

        LOGGER.debug("Invalid item entry %u is not in item_properties table or in Item.dbc! Please create a item_properties entry to return a valid displayId", entry);
#else
        return entry;
#endif
    }
...
void MySQLDataStore::loadCreaturePropertiesTable()
{
    ...
    for (tableiterator = CreaturePropertiesTables.begin(); tableiterator != CreaturePropertiesTables.end(); ++tableiterator)
    {
        ...
        {
            logger.failure("Additional creature_properties table `%s` has %u columns, but needs %u columns! Skipped!", table_name.c_str(), creature_properties_result->GetFieldCount());
            delete creature_properties_result;
            continue;
        }

        logger.info("MySQLDataLoads : Table `%s` has %u columns", table_name.c_str(), creature_properties_result->GetFieldCount());
        ...
        do
        {
            ...
            if (creatureProperties.Male_DisplayID != 0)
            {
                ...
                {
                    logger.debug("Table %s includes invalid Male_DisplayID %u for npc entry: %u. Set to 0!", (*tableiterator).c_str(), creatureProperties.Male_DisplayID, entry);
            ...
            if (creatureProperties.Female_DisplayID != 0)
            {
                ...
                if (creature_display == nullptr)
                {
                    logger.debug("Table %s includes invalid Female_DisplayID %u for npc entry: %u. Set to 0!", (*tableiterator).c_str(), creatureProperties.Female_DisplayID, entry);
            ...
            if (creatureProperties.Male_DisplayID2 != 0)
            {
                ...
                if (creature_display == nullptr)
                {
                    logger.debug("Table %s includes invalid Male_DisplayID2 %u for npc entry: %u. Set to 0!", (*tableiterator).c_str(), creatureProperties.Male_DisplayID2, entry);
            ...
            if (creatureProperties.Female_DisplayID2 != 0)
            {
                ...
                if (creature_display == nullptr)
                {
                    logger.debug("Table %s includes invalid Female_DisplayID2 %u for npc entry: %u. Set to 0!", (*tableiterator).c_str(), creatureProperties.Female_DisplayID2, entry);
            ...
            {
                logger.failure("Table `%s` MinHealth = 0 is not a valid value! Default set to 1 for entry: %u.", table_name.c_str(), entry);
            ...
            {
                logger.failure("Table `%s` MaxHealth = 0 is not a valid value! Default set to 1 for entry: %u.", table_name.c_str(), entry);
            ...
            {
                logger.failure("Table `%s` AttackType: %u is not a valid value! Default set to 0 for entry: %u.", table_name.c_str(), fields[10].GetUInt32(), entry);
            ...
            for (uint8_t i = 0; i < creatureMaxProtoSpells; ++i)
            {
                ...
                if (creatureProperties.AISpells[i] != 0)
                {
                    ...
                    if (sp == nullptr)
                    {
                        ...
                        logger.debug("spell %u in table %s column spell%u for creature entry: %u is not a valid spell!", creatureProperties.AISpells[i], table_name.c_str(), spell_number + 1, entry);
                        continue;
                    }
                    else
                    {
    ...
    logger.info("MySQLDataLoads : Loaded %u creature proto data in %u ms!", creature_properties_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadGameObjectPropertiesTable()
{
    ...
    for (tableiterator = GameObjectPropertiesTables.begin(); tableiterator != GameObjectPropertiesTables.end(); ++tableiterator)
    {
        ...
        {
            logger.failure("Additional gameobject_properties table `%s` has %u columns, but needs %u columns! Skipped!", table_name.c_str(), gameobject_properties_result->GetFieldCount());
            delete gameobject_properties_result;
            continue;
        }

        logger.info("MySQLDataLoads : Table `%s` has %u columns", table_name.c_str(), gameobject_properties_result->GetFieldCount());
        ...
        do
        {
            ...
            for (uint8_t i = 0; i < 6; ++i)
            {
                ...
                if (quest_item_entry != 0)
                {
                    ...
                    {
                        logger.failure("Table `%s` questitem%u : %u is not a valid item! Default set to 0 for entry: %u.", table_name.c_str(), i, quest_item_entry, entry);
    ...
    logger.info("MySQLDataLoads : Loaded %u gameobject data in %u ms!", gameobject_properties_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadQuestPropertiesTable()
{
    ...
    for (tableiterator = QuestPropertiesTables.begin(); tableiterator != QuestPropertiesTables.end(); ++tableiterator)
    {
        ...
        logger.info("MySQLDataLoads : Table `%s` has %u columns", table_name.c_str(), quest_result->GetFieldCount());
            ...
            for (uint8_t i = 0; i < 4; ++i)
            {
                ...
                if (questInfo.required_mob_or_go[i] != 0)
                {
                    if (questInfo.required_mob_or_go[i] > 0)
                    {
                        ...
                        {
                            logger.failure("Quest %u has `ReqCreatureOrGOId%d` = %i but creature with entry %u does not exist in creature_properties table!",
                    ...
                    else
                    {
                        ...
                        {
                            logger.failure("Quest %u has `ReqCreatureOrGOId%d` = %i but gameobject %u does not exist in gameobject_properties table!",
                                     entry, i, questInfo.required_mob_or_go[i], -questInfo.required_mob_or_go[i]);
                        }
                    }
                }
    ...
    logger.info("MySQLDataLoads : Loaded %u quest_properties data in %u ms!", quest_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadGameObjectQuestItemBindingTable()
{
    ...
    if (gameobject_quest_item_result != nullptr)
    {
        do
        {
            ...
            {
                logger.debug("Table `gameobject_quest_item_binding` includes data for invalid gameobject_properties entry: %u. Skipped!", entry);
                continue;
            }
            ...
            if (quest == nullptr)
            {
                logger.debug("Table `gameobject_quest_item_binding` includes data for invalid quest_properties : %u. Skipped!", quest_entry);
                continue;
            }
            else
            {
    ...
    logger.info("MySQLDataLoads : Loaded %u data from `gameobject_quest_item_binding` table in %u ms!", gameobject_quest_item_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadGameObjectQuestPickupBindingTable()
{
    ...
    if (gameobject_quest_pickup_result != nullptr)
    {
        do
        {
            ...
            if (gameobject_properties == nullptr)
            {
                logger.debug("Table `gameobject_quest_pickup_binding` includes data for invalid gameobject_properties entry: %u. Skipped!", entry);
                continue;
            }
            ...
            if (quest == nullptr)
            {
                logger.debug("Table `gameobject_quest_pickup_binding` includes data for invalid quest_properties : %u. Skipped!", quest_entry);
                continue;
            }
            else
            {
    ...
    logger.info("MySQLDataLoads : Loaded %u data from `gameobject_quest_pickup_binding` table in %u ms!", gameobject_quest_pickup_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadCreatureDifficultyTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `creature_difficulty` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `creature_difficulty` has %u columns", creature_difficulty_result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u creature difficulties info from `creature_difficulty` table in %u ms!", creature_difficulty_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadDisplayBoundingBoxesTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `display_bounding_boxes` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `display_bounding_boxes` has %u columns", display_bounding_boxes_result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u display bounding info from `display_bounding_boxes` table in %u ms!", display_bounding_boxes_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadVendorRestrictionsTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `vendor_restrictions` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `vendor_restrictions` has %u columns", vendor_restricitons_result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u restrictions from `vendor_restrictions` table in %u ms!", vendor_restricitons_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadNpcTextTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `npc_text` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `npc_text` has %u columns", npc_text_result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `npc_text` table in %u ms!", npc_text_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadNpcScriptTextTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `npc_script_text` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `npc_script_text` has %u columns", npc_script_text_result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `npc_script_text` table in %u ms!", npc_script_text_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadGossipMenuOptionTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `gossip_menu_option` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `gossip_menu_option` has %u columns", gossip_menu_optiont_result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `gossip_menu_option` table in %u ms!", gossip_menu_optiont_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadGraveyardsTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `graveyards` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `graveyards` has %u columns", graveyards_result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `graveyards` table in %u ms!", graveyards_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadTeleportCoordsTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `spell_teleport_coords` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `spell_teleport_coords` has %u columns", teleport_coords_result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `spell_teleport_coords` table in %u ms!", teleport_coords_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadFishingTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `fishing` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `fishing` has %u columns", fishing_result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `fishing` table in %u ms!", fishing_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadWorldMapInfoTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `worldmap_info` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `worldmap_info` has %u columns", worldmap_info_result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `worldmap_info` table in %u ms!", world_map_info_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadZoneGuardsTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `zoneguards` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `zoneguards` has %u columns", zone_guards_result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `zoneguards` table in %u ms!", zone_guards_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadBattleMastersTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `battlemasters` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `battlemasters` has %u columns", battlemasters_result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `battlemasters` table in %u ms!", battlemasters_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadTotemDisplayIdsTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `totemdisplayids` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `totemdisplayids` has %u columns", totemdisplayids_result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `totemdisplayids` table in %u ms!", static_cast<uint32_t>(_totemDisplayIdsStore.size()), static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadSpellClickSpellsTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `spellclickspells` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `spellclickspells` has %u columns", spellclickspells_result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `spellclickspells` table in %u ms!", spellclickspells_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadWorldStringsTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `worldstring_tables` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `worldstring_tables` has %u columns", worldstring_tables_result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `worldstring_tables` table in %u ms!", worldstring_tables_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadPointsOfInterestTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `points_of_interest` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `points_of_interest` has %u columns", points_of_interest_result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `points_of_interest` table in %u ms!", points_of_interest_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadItemSetLinkedSetBonusTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `itemset_linked_itemsetbonus` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `itemset_linked_itemsetbonus` has %u columns", linked_set_bonus_result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `itemset_linked_itemsetbonus` table in %u ms!", linked_set_bonus_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadCreatureInitialEquipmentTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `creature_initial_equip` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `creature_initial_equip` has %u columns", initial_equipment_result->GetFieldCount());
    ...
    do
    {
        ...
        {
            //LOGGER.debug("Invalid creature_entry %u in table creature_initial_equip!", entry);
            continue;
        }
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `creature_initial_equip` table in %u ms!", initial_equipment_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadPlayerCreateInfoTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `playercreateinfo` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `playercreateinfo` has %u columns", player_create_info_result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `playercreateinfo` table in %u ms!", static_cast<uint32_t>(_playerCreateInfoStore.size()), static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadPlayerCreateInfoSkillsTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `playercreateinfo_skills` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `playercreateinfo_skills` has %u columns", player_create_info_skills_result->GetFieldCount());
    ...
    do
    {
        ...
        {
            logger.failure("Table `playercreateinfo_skills` includes invalid skill id %u for index %u", skill_id, player_info_index);
            continue;
        }
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `playercreateinfo_skills` table in %u ms!", player_create_info_skills_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadPlayerCreateInfoSpellsTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `playercreateinfo_spells` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `playercreateinfo_spells` has %u columns", player_create_info_spells_result->GetFieldCount());
    ...
    do
    {
        ...
        {
            logger.failure("Table `playercreateinfo_spells` includes invalid spell %u for index %u", spell_id, player_info_index);
            continue;
        }
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `playercreateinfo_spells` table in %u ms!", player_create_info_spells_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadPlayerCreateInfoItemsTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `playercreateinfo_items` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `playercreateinfo_items` has %u columns", player_create_info_items_result->GetFieldCount());
    ...
    do
    {
        ...
        {
            logger.failure("Table `playercreateinfo_items` includes invalid item %u for index %u", item_id, player_info_index);
            continue;
        }
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `playercreateinfo_items` table in %u ms!", player_create_info_items_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadPlayerCreateInfoBarsTable(uint32_t player_info_index)
{
    ...
    {
        logger.info("MySQLDataLoads : Table `playercreateinfo_bars` has no data for class %u", uint32_t(playerCreateInfo.class_));
        return;
    }

    //LOGGER.info("MySQLDataLoads : Table `playercreateinfo_bars` has %u columns", player_create_info_bars_result->GetFieldCount());
...
void MySQLDataStore::loadPlayerXpToLevelTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `player_xp_for_level` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `playercreateinfo_bars` has %u columns", player_xp_to_level_result->GetFieldCount());
    ...
    do
    {
        ...
        {
            logger.failure("Table `player_xp_for_level` includes invalid xp definitions for level %u which is higher than the defined levelcap in your config file! <skipped>", current_level);
            continue;
        }
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `player_xp_for_level` table in %u ms!", player_xp_to_level_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));

    if (player_xp_to_level_count < (worldConfig.player.playerLevelCap - 1))
        logger.failure("Table `player_xp_for_level` includes definitions for %u level, but your defined level cap is %u!", player_xp_to_level_count, worldConfig.player.playerLevelCap);
}
...
void MySQLDataStore::loadSpellOverrideTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `spelloverride` is empty!");
        return;
    }
    ...
    do
    {
        ...
        if (spellid_for_overrideid_result != nullptr)
        {
            do
            {
                ...
                if (spell == nullptr)
                {
                    logger.failure("Table `spelloverride` includes invalid spellId %u for overrideId %u! <skipped>", spellid, distinct_override_id);
                    continue;
                }
    ...
    logger.info("MySQLDataLoads : %u spell overrides loaded.", static_cast<uint32_t>(_spellOverrideIdStore.size()));
}
...
void MySQLDataStore::loadNpcGossipTextIdTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `npc_gossip_textid` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `npc_gossip_textid` has %u columns", npc_gossip_textid_result->GetFieldCount());
    ...
    do
    {
        ...
        if (creature_properties == nullptr)
        {
            logger.debug("Table `npc_gossip_textid` includes invalid creatureid %u! <skipped>", entry);
            continue;
        }
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `npc_gossip_textid` table in %u ms!", npc_gossip_textid_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadPetLevelAbilitiesTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `pet_level_abilities` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `pet_level_abilities` has %u columns", pet_level_abilities_result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `pet_level_abilities` table in %u ms!", pet_level_abilities_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));

    if (pet_level_abilities_count < worldConfig.player.playerLevelCap)
        logger.failure("Table `pet_level_abilities` includes definitions for %u level, but your defined level cap is %u!", pet_level_abilities_count, worldConfig.player.playerLevelCap);
}
...
void MySQLDataStore::loadBroadcastTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `worldbroadcast` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `worldbroadcast` has %u columns", broadcast_result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `worldbroadcast` table in %u ms!", broadcast_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadAreaTriggerTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `areatriggers` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `areatriggers` has %u columns", area_trigger_result->GetFieldCount());
    ...
    do
    {
        ...
        {
            logger.debug("AreaTrigger : Area trigger (ID:%u) does not exist in `AreaTrigger.dbc`.", areaTrigger.id);
            continue;
        }
        ...
        if (!map_entry)
        {
            logger.debug("AreaTrigger : Area trigger (ID:%u) target map (ID: %u) does not exist in `Map.dbc`.", areaTrigger.id, areaTrigger.mapId);
            continue;
        }
        ...
        {
            logger.debug("AreaTrigger : Area trigger (ID:%u) target coordinates not provided.", areaTrigger.id);
            continue;
        }
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `areatriggers` table in %u ms!", areaTrigger_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadWordFilterCharacterNames()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `wordfilter_character_names` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `wordfilter_character_names` has %u columns", filter_character_names_result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `wordfilter_character_names` table in %u ms!", filter_character_names_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadWordFilterChat()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `wordfilter_chat` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `wordfilter_chat` has %u columns", filter_chat_result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `wordfilter_chat` table in %u ms!", filter_chat_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadCreatureFormationsTable()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `creature_formations` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `creature_formations` has %u columns", creature_formations_result->GetFieldCount());
    ...
    do
    {
        ...
        {
            logger.failure("Table `creature_formations` includes formation data for invalid spawn id %u. Skipped!", spawnId);
            continue;
        }
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `creature_formations` table in %u ms!", formations_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadLocalesCreature()
{
    ...
    if (result == nullptr)
    {
        logger.info("MySQLDataLoads : Table `locales_creature` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `locales_creature` has %u columns", result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `locales_creature` table in %u ms!", load_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadLocalesGameobject()
{
    ...
    if (result == nullptr)
    {
        logger.info("MySQLDataLoads : Table `locales_gameobject` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `locales_gameobject` has %u columns", result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `locales_gameobject` table in %u ms!", load_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadLocalesGossipMenuOption()
{
    ...
    if (result == nullptr)
    {
        logger.info("MySQLDataLoads : Table `locales_gossip_menu_option` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `locales_gossip_menu_option` has %u columns", result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `locales_gossip_menu_option` table in %u ms!", load_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadLocalesItem()
{
    ...
    if (result == nullptr)
    {
        logger.info("MySQLDataLoads : Table `locales_item` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `locales_item` has %u columns", result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `locales_item` table in %u ms!", load_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadLocalesItemPages()
{
    ...
    if (result == nullptr)
    {
        logger.info("MySQLDataLoads : Table `locales_item_pages` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `locales_item_pages` has %u columns", result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `locales_item_pages` table in %u ms!", load_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadLocalesNPCMonstersay()
{
    ...
    {
        logger.info("MySQLDataLoads : Table `locales_npc_monstersay` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `locales_npc_monstersay` has %u columns", local_monstersay_result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `locales_npc_monstersay` table in %u ms!", local_monstersay_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadLocalesNpcScriptText()
{
    ...
    if (result == nullptr)
    {
        logger.info("MySQLDataLoads : Table `locales_npc_script_text` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `locales_npc_script_text` has %u columns", result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `locales_npc_script_text` table in %u ms!", load_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadLocalesNpcText()
{
    ...
    if (result == nullptr)
    {
        logger.info("MySQLDataLoads : Table `locales_npc_text` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `locales_npc_text` has %u columns", result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `locales_npc_text` table in %u ms!", load_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadLocalesQuest()
{
    ...
    if (result == nullptr)
    {
        logger.info("MySQLDataLoads : Table `locales_quest` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `locales_quest` has %u columns", result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `locales_quest` table in %u ms!", load_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadLocalesWorldbroadcast()
{
    ...
    if (result == nullptr)
    {
        logger.info("MySQLDataLoads : Table `locales_worldbroadcast` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `locales_worldbroadcast` has %u columns", result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `locales_worldbroadcast` table in %u ms!", load_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadLocalesWorldmapInfo()
{
    ...
    if (result == nullptr)
    {
        logger.info("MySQLDataLoads : Table `locales_worldmap_info` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `locales_worldmap_info` has %u columns", result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `locales_worldmap_info` table in %u ms!", load_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadLocalesWorldStringTable()
{
    ...
    if (result == nullptr)
    {
        logger.info("MySQLDataLoads : Table `locales_worldstring_table` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `locales_worldstring_table` has %u columns", result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `locales_worldstring_table` table in %u ms!", load_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadNpcMonstersayTable()
{
    ...
    if (result == nullptr)
    {
        logger.info("MySQLDataLoads : Table `npc_monstersay` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `npc_monstersay` has %u columns", result->GetFieldCount());
    ...
    do
    {
        ...
        {
            logger.debug("Duplicate npc_monstersay event %u for entry %u, skipping", creatureEvent, entry);
            continue;
        }
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `npc_monstersay` table in %u ms!", load_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
//    if (result == nullptr)
//    {
//        LOGGER.info("MySQLDataLoads : Table `petdefaultspells` is empty!");
//        return;
//    }
//
//    LOGGER.info("MySQLDataLoads : Table `petdefaultspells` has %u columns", result->GetFieldCount());
//
//    uint32_t load_count = 0;
//    do
//    {
...
//    delete result;
//
//    LOGGER.info("MySQLDataLoads : Loaded %u rows from `petdefaultspells` table in %u ms!", load_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
//}
...
void MySQLDataStore::loadProfessionDiscoveriesTable()
{
    ...
    if (result == nullptr)
    {
        logger.info("MySQLDataLoads : Table `professiondiscoveries` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `professiondiscoveries` has %u columns", result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `professiondiscoveries` table in %u ms!", load_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadTransportDataTable()
{
    ...
    if (result == nullptr)
    {
        logger.info("MySQLDataLoads : Table `transport_data` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `transport_data` has %u columns", result->GetFieldCount());
    ...
    do
    {
        ...
        {
            logger.failure("Transport entry: %u, will not be loaded, gameobject_properties missing", entry);
            continue;
        }

        if (gameobject_info->type != GAMEOBJECT_TYPE_MO_TRANSPORT)
        {
            logger.failure("Transport entry: %u, will not be loaded, gameobject_properties type wrong", entry);
            continue;
        }
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `transport_data` table in %u ms!", load_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadTransportEntrys()
{
    ...
    if (result == nullptr)
    {
        logger.info("MySQLDataLoads : Loaded 0 transport templates. DB table `gameobject_properties` has no transports!");
        return;
    }
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `transport_entrys` table in %u ms!", load_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadGossipMenuItemsTable()
{
    ...
    if (result == nullptr)
    {
        logger.info("MySQLDataLoads : Table `gossip_menu` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `gossip_menu` has %u columns", result->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `gossip_menu` table in %u ms!", load_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
    ...
    {
        logger.info("MySQLDataLoads : Table `gossip_menu_items` is empty!");
        return;
    }

    logger.info("MySQLDataLoads : Table `gossip_menu_items` has %u columns", resultItems->GetFieldCount());
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `gossip_menu_items` table in %u ms!", load_count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadCreatureSpawns()
{
    ...
    for (std::set<std::string>::iterator tableiterator = CreatureSpawnsTables.begin(); tableiterator != CreatureSpawnsTables.end(); ++tableiterator)
    {
        ...
        if (creature_spawn_result)
        {
            ...
            {
                logger.failure("Table `%s` has %u columns, but needs %u columns! Skipped!", (*tableiterator).c_str(), creature_spawn_fields, CREATURE_SPAWNS_FIELDCOUNT);
                continue;
            }
            else
            {
                do
                {
                    ...
                    if (creature_properties == nullptr)
                    {
                        logger.failure("Creature spawn ID: %u has invalid entry: %u which is not in creature_properties table! Skipped loading.", cspawn->id, creature_entry);
                        continue;
                    }
                    ...
                    if (cspawn->displayid != 0)
                    {
                        ...
                        if (!creature_display)
                        {
                            logger.failure("Table %s includes invalid displayid %u for npc entry: %u, spawn_id: %u. Set to a random modelid!", (*tableiterator).c_str(), cspawn->displayid, cspawn->entry, cspawn->id);
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `creature_spawns` table in %u ms!", count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
void MySQLDataStore::loadGameobjectSpawns()
{
    ...
    for (std::set<std::string>::iterator tableiterator = GameObjectSpawnsTables.begin(); tableiterator != GameObjectSpawnsTables.end(); ++tableiterator)
    {
        ...
        if (gobject_spawn_result)
        {
            ...
            {
                logger.failure("Table `%s` has %u columns, but needs %u columns! Skipped!", (*tableiterator).c_str(), gobject_spawn_fields, GO_SPAWNS_FIELDCOUNT);
                continue;
            }
            else
            {
                do
                {
                    ...
                    if (gameobject_info == nullptr)
                    {
                        logger.failure("Gameobject spawn ID: %u has invalid entry: %u which is not in gameobject_properties table! Skipped loading.", go_spawn->id, gameobject_entry);
                        continue;
                    }
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `gameobject_spawns` table in %u ms!", count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
...
void MySQLDataStore::loadRecallTable()
{
    ...
    logger.info("MySQLDataLoads : Loaded %u rows from `recall` table in %u ms!", count, static_cast<uint32_t>(Util::GetTimeDifferenceToNow(startTime)));
}
