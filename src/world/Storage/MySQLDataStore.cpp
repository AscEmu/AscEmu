...
void MySQLDataStore::LoadAdditionalTableConfig()
{
    ...
    // get config
    std::string strData = sWorld.settings.startupSettings.additionalTableLoads;
    if (strData.empty())
        return;
...
void MySQLDataStore::LoadPlayerXpToLevelTable()
{
    ...
    _playerXPperLevelStore.resize(sWorld.settings.optionalSettings.playerLevelCap);

    for (uint32 level = 0; level < sWorld.settings.optionalSettings.playerLevelCap; ++level)
    ...
    do
    {
        ...
        if (current_level >= sWorld.settings.optionalSettings.playerLevelCap)
        {
    ...
    if (player_xp_to_level_count < (sWorld.settings.optionalSettings.playerLevelCap - 1))
        LOG_ERROR("Table `player_xp_for_level` includes definitions for %u level, but your defined level cap is %u!", player_xp_to_level_count, sWorld.settings.optionalSettings.playerLevelCap);
}
...
void MySQLDataStore::LoadPetLevelAbilitiesTable()
{
    ...
    if (pet_level_abilities_count < sWorld.settings.optionalSettings.playerLevelCap)
        LOG_ERROR("Table `pet_level_abilities` includes definitions for %u level, but your defined level cap is %u!", pet_level_abilities_count, sWorld.settings.optionalSettings.playerLevelCap);
}
