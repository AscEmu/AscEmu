...
Spell* SpellMgr::newSpell(Object* caster, SpellInfo const* info, bool triggered, Aura* aur)
{
    if (info == nullptr)
    {
        LOGGER.failure("You tried to create a Spell without SpellInfo. This is not possible!");
        return nullptr;
    }
...
Aura* SpellMgr::newAura(SpellInfo const* spellInfo, int32_t duration, Object* caster, Unit* target, bool temporary /*= false*/, Item* i_caster /*= nullptr*/)
{
    ...
    {
        LOGGER.failure("You tried to create an Aura without SpellInfo. This is not possible!");
        return nullptr;
    }
...
void SpellMgr::addSpellById(const uint32_t spellId, SpellScriptLinker spellScript)
{
    ...
    if (spellInfo == nullptr)
    {
        LOGGER.failure("SpellMgr::addSpellById : Unknown spell id %u tried to register a spell script, skipped", spellId);
        return;
    }
...
void SpellMgr::addAuraById(const uint32_t spellId, AuraScriptLinker auraScript)
{
    ...
    if (spellInfo == nullptr)
    {
        LOGGER.failure("SpellMgr::addAuraById : Unknown spell id %u tried to register an aura script, skipped", spellId);
        return;
    }
...
void SpellMgr::loadSpellCoefficientOverride()
{
    ...
    if (result == nullptr)
    {
        LOGGER.debug("SpellMgr::loadSpellCoefficientOverride : Your `spell_coefficient_override` table is empty!");
        return;
    }
    ...
    do
    {
        ...
        if (spellInfo == nullptr)
        {
            LOGGER.failure("Table `spell_coefficient_override` has unknown spell entry %u, skipped", fields[0].GetUInt32());
            continue;
        }
    ...
    LOGGER.info("Loaded %u override values from `spell_coefficient_override` table", overridenCoeffs);
}
...
void SpellMgr::loadSpellCustomOverride()
{
    ...
    if (result == nullptr)
    {
        LOGGER.debug("SpellMgr::loadSpellCustomOverride : Your `spell_custom_override` table is empty!");
        return;
    }
    ...
    do
    {
        ...
        if (spellInfo == nullptr)
        {
            LOGGER.failure("Table `spell_custom_override` has unknown spell entry %u, skipped", fields[0].GetUInt32());
            continue;
        }
    ...
    LOGGER.info("Loaded %u override values from `spell_custom_override` table", overridenSpells);
}
...
void SpellMgr::loadSpellAIThreat()
{
    ...
    if (result == nullptr)
    {
        LOGGER.debug("SpellMgr::loadSpellAIThreat : Your `ai_threattospellid` table is empty!");
        return;
    }
    ...
    do
    {
        ...
        if (spellInfo == nullptr)
        {
            LOGGER.failure("Table `ai_threattospellid` has invalid spell entry %u, skipped", spellId);
            continue;
        }
    ...
    LOGGER.info("Loaded %u spell ai threat", threatCount);
}
...
void SpellMgr::loadSpellEffectOverride()
{
    ...
    if (result == nullptr)
    {
        LOGGER.debug("SpellMgr::loadSpellEffectOverride : Your `spell_effects_override` table is empty!");
        return;
    }
    ...
    do
    {
        ...
        if (spellInfo == nullptr)
        {
            LOGGER.failure("Table `spell_effects_override` has invalid spell entry %u, skipped", seo_SpellId);
            continue;
        }
    ...
    LOGGER.info("Loaded %u spell effect overrides", overridenEffects);
}
...
void SpellMgr::loadSpellAreas()
{
    ...
    if (result == nullptr)
    {
        LOGGER.debug("SpellMgr::loadSpellAreas : Your `spell_area` table is empty!");
        return;
    }
    ...
    do
    {
        ...
        if (duplicate)
        {
            LOGGER.failure("Table `spell_area` has duplicate spell entry (%u) with similiar requirements, skipped", spellId);
            continue;
        }
        ...
        if (spellArea.areaId > 0)
        {
            ...
            {
                LOGGER.failure("Table `spell_area` has invalid area id %u for spell entry %u, skipped", spellArea.areaId, spellId);
                continue;
            }
        }
        ...
        if (spellArea.questStart > 0)
        {
            ...
            {
                LOGGER.failure("Table `spell_area` has invalid quest id %u for spell entry %u, skipped", spellArea.questStart, spellId);
                continue;
            }
        }
        ...
        if (spellArea.questEnd > 0)
        {
            ...
            if (endQuest == nullptr)
            {
                LOGGER.failure("Table `spell_area` has invalid quest id %u for spell entry %u, skipped", spellArea.questEnd, spellId);
                continue;
            }
            ...
            {
                LOGGER.failure("Table `spell_area` has quest (id %u) requirement for spell entry %u for start and end at the same time, skipped", spellArea.questEnd, spellId);
                continue;
            }
        }
        ...
        if (spellArea.auraSpell != 0)
        {
            ...
            if (spellInfo == nullptr)
            {
                LOGGER.failure("Table `spell_area` has invalid aura spell entry %u for spell %u, skipped", abs(spellArea.auraSpell), spellId);
                continue;
            }

            if (static_cast<uint32_t>(abs(spellArea.auraSpell)) == spellArea.spellId)
            {
                LOGGER.failure("Table `spell_area` has aura spell requirements for itself (id %u), skipped", spellId);
                continue;
            }
            ...
            if (spellArea.autoCast && spellArea.auraSpell > 0)
            {
                ...
                if (chain)
                {
                    LOGGER.failure("Table `spell_area` has aura spell (id %u) requirement that itself was autocasted from an aura, skipping", spellId);
                    continue;
                }
        ...
        {
            LOGGER.failure("Table `spell_area` has invalid racemask for spell entry %u, skipped", spellId);
            continue;
        }

        if (spellArea.gender != GENDER_NONE && spellArea.gender != GENDER_MALE && spellArea.gender != GENDER_FEMALE)
        {
            LOGGER.failure("Table `spell_area` has invalid gender for spell entry %u, skipped", spellId);
            continue;
        }
    ...
    LOGGER.info("Loaded %u spell area requirements", areaCount);
}
