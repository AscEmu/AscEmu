...
void Aura::SpellAuraPeriodicDamage(bool apply)
{
    if (apply)
    {
        ...
        sEventMgr.AddEvent(this, &Aura::EventPeriodicDamage, (uint32)dmg,
                           EVENT_AURA_PERIODIC_DAMAGE, GetSpellInfo()->getEffectAmplitude(static_cast<uint8_t>(mod->i)), 0, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
...
void Aura::EventPeriodicDamage(uint32 amount)
{
    ...
    if (m_target->GetGUID() != m_casterGuid)	//don't use resist when cast on self-- this is some internal stuff
    {
        if (c != nullptr)
        {
            uint32 amp = m_spellInfo->getEffectAmplitude(static_cast<uint8_t>(mod->i));
            if (!amp)
...
void Aura::SpellAuraPeriodicHeal(bool apply)
{
    if (apply)
    {
        ...
        if (val > 0)
        {
            sEventMgr.AddEvent(this, &Aura::EventPeriodicHeal, (uint32)val, EVENT_AURA_PERIODIC_HEAL, GetSpellInfo()->getEffectAmplitude(static_cast<uint8_t>(mod->i)), 0, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
...
void Aura::EventPeriodicHeal(uint32 amount)
{
    ...
    int amp = m_spellInfo->getEffectAmplitude(static_cast<uint8_t>(mod->i));
    if (!amp)
...
void Aura::SpellAuraConvertRune(bool apply)
{
    ...
    if (apply)
    {
        for (uint8_t i = 0; i < MAX_RUNES; ++i)
        {
            ...
            {
                dk->ConvertRune(i, static_cast<uint8_t>(GetSpellInfo()->getEffectMiscValueB(static_cast<uint8_t>(mod->i))));
            }
        }
    }
    else
