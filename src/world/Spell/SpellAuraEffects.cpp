...
{
    sLogger.debugAuraEffect("Aura::applyModifiers : unknown aura id {} for spell id {}.", aurEff->getAuraEffectType(), getSpellId());
}
...
void Aura::spellAuraEffectPeriodicDamage(AuraEffectModifier* aurEff, bool apply)
{
    if (apply)
    {
        ...
        if (casterUnit != nullptr && !aurEff->isEffectDamageStatic())
            damage = casterUnit->applySpellDamageBonus(casterUnit, getSpellInfo(), aurEff->getEffectIndex(), aurEff->getEffectFloatDamage(), false, nullptr, aurEff);
...
void Aura::spellAuraEffectDummy(AuraEffectModifier* aurEff, bool apply)
{
    ...
    sLogger.debugAuraEffect("Aura::spellAuraEffectDummy : Spell {} ({}) has a dummy aura effect, but no handler for it.", m_spellInfo->getId(), m_spellInfo->getName());
}
    ...
    if (apply)
    {
        ...
        if (casterUnit != nullptr)
        {
            ...
            if (!aurEff->isEffectDamageStatic())
                heal = casterUnit->applySpellHealingBonus(casterUnit, getSpellInfo(), aurEff->getEffectIndex(), aurEff->getEffectFloatDamage(), false, nullptr, aurEff);
        }
...
void Aura::spellAuraEffectPeriodicHealPct(AuraEffectModifier* aurEff, bool apply)
{
    if (apply)
    {
        ...
        const auto casterUnit = GetUnitCaster();
        if (casterUnit != nullptr && !aurEff->isEffectDamageStatic())
            aurEff->setEffectDamage(casterUnit->applySpellHealingBonus(casterUnit, getSpellInfo(), aurEff->getEffectIndex(), aurEff->getEffectFloatDamage(), false, nullptr, aurEff));
...
void Aura::spellAuraEffectTransform(AuraEffectModifier* aurEff, bool apply)
{
    if (apply)
    {
        ...
        {
            sLogger.debugAuraEffect("Aura::spellAuraEffectTransform : unknown creature entry {} in misc value for spell {}.", aurEff->getEffectMiscValue(), getSpellId());
            return;
        }
        ...
        if (displayId == 0)
        {
            sLogger.debugAuraEffect("Aura::spellAuraEffectTransform : creature entry {} has no display id for spell {}.", properties->Id, getSpellId());
            return;
        }
...
void Aura::spellAuraEffectPeriodicHealthFunnel(AuraEffectModifier* aurEff, bool apply)
{
    if (apply)
    {
        ...
        const auto casterUnit = GetUnitCaster();
        if (casterUnit != nullptr && !aurEff->isEffectDamageStatic())
            aurEff->setEffectDamage(casterUnit->applySpellDamageBonus(casterUnit, getSpellInfo(), aurEff->getEffectIndex(), aurEff->getEffectFloatDamage(), false, nullptr, aurEff));
...
void Aura::spellAuraEffectPeriodicDamagePercent(AuraEffectModifier* aurEff, bool apply)
{
    if (apply)
    {
        aurEff->setEffectDamage((aurEff->getEffectBaseDamage() / 100.0f * getOwner()->getMaxHealth()) * getStackCount());
