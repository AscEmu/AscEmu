...
{
    logger.debug("Spells: Unhandled spell effect %u in spell %u.", getSpellInfo()->getEffect(effIndex), getSpellInfo()->getId());
}
...
void Spell::SpellEffectDummy(uint8_t effectIndex) // Dummy(Scripted events)
{
    ...
    logger.debug("Spell ID: %u (%s) has a dummy effect index (%hhu) but no handler for it.", m_spellInfo->getId(), m_spellInfo->getName().c_str(), effectIndex);
}
...
void Spell::SpellEffectTeleportUnits(uint8_t effectIndex)    // Teleport Units
{
    ...
    if (m_spellInfo->hasCustomFlagForEffect(effectIndex, TELEPORT_TO_COORDINATES))
    {
        ...
        {
            logger.failure("Spell %u (%s) has a TELEPORT TO COORDINATES effect, but has no coordinates to teleport to. ", spellId, m_spellInfo->getName().c_str());
            return;
        }
    ...
    logger.failure("Unhandled Teleport effect Index %hhu for Spell %u (%s).", effectIndex, m_spellInfo->getId(), m_spellInfo->getName().c_str());
}
...
void Spell::SpellEffectCreateItem(uint8_t effectIndex)
{
    ...
    {
        logger.failure("Spell %u (%s) has a create item effect but no player target!", spellid, m_spellInfo->getName().c_str());
        return;
    }
    ...
    if (itemid == 0)
    {
        logger.failure("Spell %u (%s) has a create item effect but no itemid to add, Spell needs to be fixed!", spellid, m_spellInfo->getName().c_str());
        return;
    }
    ...
    {
        logger.failure("Spell %u (%s) has a create item effect but the itemid is invalid!", spellid, m_spellInfo->getName().c_str());
        return;
    }
    ...
    if (p_caster != nullptr)
    {
        ...
        if (p_caster != nullptr)
        {
            ...
            if ((learn_spell != 0) && (p_caster->getLevel() > 60) && !p_caster->HasSpell(learn_spell) && Util::checkChance(cast_chance))
            {
                ...
                {
                    logger.failure("Spell %u (%s) Effect Index %hhu tried to teach a non-existing Spell %u in %s:%u", spellid, m_spellInfo->getName().c_str(), effectIndex, learn_spell, __FILE__, __LINE__);
                }
            }
        }
...
void Spell::SpellEffectWeapon(uint8_t /*effectIndex*/)
{
    ...
    switch (this->getSpellInfo()->getId())
    {
        ...
        {
            skill = 0;
            logger.debug("WARNING: Could not determine skill for spell id %d (SPELL_EFFECT_WEAPON)", this->getSpellInfo()->getId());
        }
        break;
    }
...
void Spell::SpellEffectSummon(uint8_t effectIndex)
{
    ...
    {
        logger.failure("No SummonPropertiesEntry for Spell %u (%s)", m_spellInfo->getId(), m_spellInfo->getName().c_str());
        return;
    }
    ...
    if (cp == nullptr)
    {
        logger.failure("Spell %u (%s) tried to summon creature %u without database data", m_spellInfo->getId(), m_spellInfo->getName().c_str(), entry);
        return;
    }
    ...
    logger.failure("Unknown summon type in summon property %u in spell %u %s", summonpropid, m_spellInfo->getId(), m_spellInfo->getName().c_str());
}
...
void Spell::SpellEffectSummonWild(uint8_t effectIndex)  // Summon Wild
{
    ...
    {
        logger.failure("Warning : Missing summon creature template %u used by spell %u!", cr_entry, getSpellInfo()->getId());
        return;
    }
    float x, y, z;
...
void Spell::SpellEffectTriggerMissile(uint8_t effectIndex) // Trigger Missile
{
    ...
    if (spellid == 0)
    {
        logger.failure("Spell %u (%s) has a trigger missle effect index (%hhu) but no trigger spell ID. Spell needs fixing.", m_spellInfo->getId(), m_spellInfo->getName().c_str(), effectIndex);
        return;
    }
    ...
    if (spInfo == nullptr)
    {
        logger.failure("Spell %u (%s) has a trigger missle effect index (%hhu) but has an invalid trigger spell ID. Spell needs fixing.", m_spellInfo->getId(), m_spellInfo->getName().c_str(), effectIndex);
        return;
    }
...
void Spell::SpellEffectSummonObject(uint8_t effectIndex)
{
    ...
    if (info == nullptr)
    {
        logger.failure("Spell %u ( %s ) Effect Index %u tried to summon a GameObject with ID %u. GameObject is not in the database.", m_spellInfo->getId(), m_spellInfo->getName().c_str(), effectIndex, entry);
        return;
    }
...
void Spell::SpellEffectEnchantItem(uint8_t effectIndex) // Enchant Item Permanent
{
    ...
    {
        logger.failure("Invalid enchantment entry %u for Spell %u", getSpellInfo()->getEffectMiscValue(effectIndex), getSpellInfo()->getId());
        return;
    }
...
void Spell::SpellEffectEnchantItemTemporary(uint8_t effectIndex)  // Enchant Item Temporary
{
    ...
    if (Duration == 0)
    {
        logger.failure("Spell %u (%s) has no enchantment duration. Spell needs to be fixed!", m_spellInfo->getId(), m_spellInfo->getName().c_str());
        return;
    }

    if (EnchantmentID == 0)
    {
        logger.failure("Spell %u (%s) has no enchantment ID. Spell needs to be fixed!", m_spellInfo->getId(), m_spellInfo->getName().c_str());
        return;
    }
    ...
    {
        logger.failure("Invalid enchantment entry %u for Spell %u", EnchantmentID, getSpellInfo()->getId());
        return;
    }
...
void Spell::SpellEffectSendEvent(uint8_t effectIndex) //Send Event
{
    ...
    logger.debug("Spell ID: %u (%s) has a scripted effect index (%u) but no handler for it.", m_spellInfo->getId(), m_spellInfo->getName().c_str(), effectIndex);
...
void Spell::SpellEffectClearQuest(uint8_t effectIndex)
{
    if (playerTarget == nullptr)
    {
        logger.failure("Spell %u (%s) was not casted on Player, but Spell requires Player to be a target.", m_spellInfo->getId(), m_spellInfo->getName().c_str());
        return;
    }
...
void Spell::SpellEffectScriptEffect(uint8_t effectIndex) // Script Effect
{
    ...
    logger.failure("Spell ID: %u (%s) has a scripted effect index (%hhu) but no handler for it.", m_spellInfo->getId(), m_spellInfo->getName().c_str(), effectIndex);
}
...
void Spell::SpellEffectActivateObject(uint8_t effectIndex) // Activate Object
{
    ...
    if (!gameObjTarget)
    {
        logger.failure("Spell %u (%s) effect %hhu not handled because no target was found. ", m_spellInfo->getId(), m_spellInfo->getName().c_str(), effectIndex);
        return;
    }
...
void Spell::SpellEffectDisenchant(uint8_t /*effectIndex*/)
{
    ...
    logger.debug("Successfully disenchanted item %d", uint32(it->getEntry()));
...
void Spell::SpellEffectProspecting(uint8_t /*effectIndex*/)
{
    ...
    {
        logger.debug("Successfully prospected item %d", uint32(itemTarget->getEntry()));
        p_caster->SendLoot(itemTarget->getGuid(), LOOT_PROSPECTING, p_caster->GetMapId());
    }
    else // this should never happen either
    {
        logger.debug("Prospecting failed, item %d has no loot", uint32(itemTarget->getEntry()));
        sendCastResult(SPELL_FAILED_CANT_BE_PROSPECTED);
    }
}
...
void Spell::SpellEffectPlayMusic(uint8_t effectIndex)
{
    ...
    if (soundid == 0)
    {
        logger.failure("Spell %u (%s) has no sound ID to play. Spell needs fixing!", m_spellInfo->getId(), m_spellInfo->getName().c_str());
        return;
    }
...
void Spell::SpellEffectForgetSpecialization(uint8_t effectIndex)
{
    ...
    logger.debug("Player %u have forgot spell %u from spell %u (caster: %u)", playerTarget->getGuidLow(), spellid, getSpellInfo()->getId(), m_caster->getGuidLow());
}
...
void Spell::SpellEffectRestorePowerPct(uint8_t effectIndex)
{
    ...
    {
        logger.failure("Unhandled power type %u in %s, report this line to devs.", power_type, __FUNCTION__);
        return;
    }
...
void Spell::SpellEffectMilling(uint8_t /*effectIndex*/)
{
    ...
    if (itemTarget->loot->items.size() > 0)
    {
        logger.debug("Successfully milled item %d", uint32(itemTarget->getEntry()));
        p_caster->SendLoot(itemTarget->getGuid(), LOOT_MILLING, p_caster->GetMapId());
    }
    else // this should never happen either
    {
        logger.debug("Milling failed, item %d has no loot", uint32(itemTarget->getEntry()));
        sendCastResult(SPELL_FAILED_CANT_BE_PROSPECTED);
    }
}
