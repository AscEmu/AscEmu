...
void Spell::SpellEffectPickpocket(uint32 i) // pickpocket
{
    ...
    unitTarget->loot.gold = float2int32((_rank + 1) * unitTarget->getLevel() * (RandomUInt(5) + 1) * sWorld.getFloatRate(RATE_MONEY));
...
void Spell::SpellEffectDisenchant(uint32 i)
{
    ...
    if (skill && skill < 60)
    {
        ...
        {
            uint32 SkillUp = float2int32(1.0f * sWorld.getFloatRate(RATE_SKILLRATE));
            if (skill + SkillUp > 60)
                SkillUp = 60 - skill;
