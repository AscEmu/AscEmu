...
Aura::Aura(SpellInfo const* proto, int32 duration, Object* caster, Unit* target, bool temporary, Item* i_caster)
{
    ...
    m_visualSlot = 0xFF;
    pSpellId = 0;
    // LOGGER.info("Aura::Constructor %u (%s) from %u.", m_spellProto->getId(), m_spellProto->Name, m_target->getGuidLow());
    m_auraSlot = 0xffff;
    m_interrupted = -1;
    m_flags = 0;
...
void Aura::EventUpdateAreaAura(uint8_t effIndex, float r)
{
    ...
    {
        LOGGER.failure("Spell %u (%s) has tried to update Area Aura targets but Spell has no Area Aura effect.", m_spellInfo->getId(), m_spellInfo->getName().c_str());
        ARCEMU_ASSERT(false);
    }
...
void Aura::SpellAuraModSchoolImmunity(AuraEffectModifier* aurEff, bool apply)
{
    ...
    if (apply)
    {
        ...
        LOGGER.debug("SpellAuraModSchoolImmunity called with misValue = %x", aurEff->getEffectMiscValue());
...
void Aura::SpellAuraProcTriggerSpell(AuraEffectModifier* aurEff, bool apply)
{
    if (apply)
    {
        ...
        if (spellId == 0)
        {
            LOGGER.debug("Warning! trigger spell is null for spell %u", getSpellInfo()->getId());
            return;
        }
        ...
        LOGGER.debug("%u is registering %u chance %u flags %u charges %u", getSpellInfo()->getId(), spellId, getSpellInfo()->getProcChance(), getSpellInfo()->getProcFlags(), getCharges());
    }
    else
    {
        // Find spell of effect to be triggered
...
void Aura::SpellAuraProcTriggerDamage(AuraEffectModifier* aurEff, bool apply)
{
    if (apply)
    {
        ...
        ds.owner = (void*)this;
        m_target->m_damageShields.push_back(ds);
        LOGGER.debug("registering dmg proc %u, school %u, flags %u, charges at least %u", ds.m_spellId, ds.m_school, ds.m_flags, m_spellInfo->getProcCharges());
    }
    else
    {
        for (std::list<struct DamageProc>::iterator i = m_target->m_damageShields.begin(); i != m_target->m_damageShields.end(); ++i)
...
void Aura::SpellAuraAddClassTargetTrigger(AuraEffectModifier* aurEff, bool apply)
{
    if (apply)
    {
        ...
        if (sp == nullptr)
        {
            LOGGER.debug("Warning! class trigger spell is null for spell %u", getSpellInfo()->getId());
            return;
        }
        ...
        LOGGER.debug("%u is registering %u chance %u flags %u charges %u", getSpellInfo()->getId(), sp->getId(), getSpellInfo()->getProcChance(), getSpellInfo()->getProcFlags(), getCharges());
    }
    else
    {
        // Find spell of effect to be triggered
    ...
    switch (aurEff->getEffectMiscValue())
    {
        ...
        case 6953:
        {
            if (apply)
            {
                ...
                {
                    LOGGER.failure("Unable to find override with overrideid: %u", aurEff->getEffectMiscValue());
                    break;
                }
        ...
        break;
        default:
            LOGGER.failure("Unknown override report to devs: %u", aurEff->getEffectMiscValue());
    };
}
...
void Aura::SpellAuraIncreaseHealingByAttribute(AuraEffectModifier* aurEff, bool apply)
{
    ...
    {
        LOGGER.failure("Aura::SpellAuraIncreaseHealingByAttribute::Unknown spell attribute type %u in spell %u.\n", aurEff->getEffectMiscValue(), getSpellId());
        return;
    }
