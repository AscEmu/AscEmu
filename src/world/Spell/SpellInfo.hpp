...
public:
    ...
    uint32_t getTotem(uint8_t idx) const
    {
        ...
        {
            LOGGER.failure("Totem index id %u is invalid!", idx);
            return 0;
        }
    ...
    int32_t getReagent(uint8_t idx) const
    {
        ...
        {
            LOGGER.failure("Reagent index id %u is invalid!", idx);
            return 0;
        }
    ...
    uint32_t getReagentCount(uint8_t idx) const
    {
        if (idx >= MAX_SPELL_REAGENTS)
        {
            LOGGER.failure("ReagentCount index id %u is invalid!", idx);
            return 0;
        }
    ...
    uint32_t getEffect(uint8_t idx) const
    {
        ...
        {
            LOGGER.failure("Effect index id %u is invalid!", idx);
            return 0;
        }
    ...
    uint32_t getEffectSpellClassMask(uint8_t idx1, uint8_t idx2) const
    {
        ...
        {
            LOGGER.failure("Totem index id %u or effect index %u is invalid!", idx1, idx2);
            return 0;
        }
    ...
    uint32_t* getEffectSpellClassMask(uint8_t idx1)
    {
        ...
        {
            LOGGER.failure("Effect index id %u is invalid!", idx1);
            return 0;
        }
    ...
    uint32_t getTotemCategory(uint8_t idx) const
    {
        ...
        {
            LOGGER.failure("TotemCategory index id %u is invalid!", idx);
            return 0;
        }
...
private:
    ...
    void setReagent(int32_t reagentId, uint8_t idx)                      // used in HackFixes.cpp
    {
        if (idx >= MAX_SPELL_REAGENTS)
        {
            LOGGER.failure("Spellreagents index id %u is invalid!", idx);
            return;
        }
    ...
    void setEffectSpellClassMask(uint32_t spellClass, uint8_t idx1, uint8_t idx2)           // used in HackFixes.cpp
    {
        if (idx1 >= MAX_SPELL_EFFECTS || idx2 >= MAX_SPELL_EFFECTS)
        {
            LOGGER.failure("Effect index id1 %u or id2 %u is invalid!", idx1, idx2);
            return;
        }
