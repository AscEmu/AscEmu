...
SpellCastResult Spell::prepare(SpellCastTargets* targets)
{
    ...
    {
        LOGGER.debug("Object " I64FMT " is casting spell ID %u while not in world", m_caster->getGuid(), getSpellInfo()->getId());
        delete this;
    ...
    if (cancastresult != SPELL_CAST_SUCCESS)
    {
        ...
        LOGGER.debug("Spell::prepare : canCast result %u for spell id %u (refer to SpellFailure.h to work out why)", cancastresult, getSpellInfo()->getId());
...
void Spell::castMe(const bool doReCheck)
{
    ...
    if (m_caster->isPlayer())
    {
        ...
        LOGGER.debug("Spell::castMe : Player guid %u casted spell %s (id %u)",
    ...
    else if (m_caster->isCreature())
    {
        ...
        LOGGER.debug("Spell::castMe : Creature guid %u (entry %u) casted spell %s (id %u)",
            creature->spawnid, creature->getEntry(), getSpellInfo()->getName().c_str(), getSpellInfo()->getId());
    }
    else
    {
        LOGGER.debug("Spell::castMe : Spell id %u casted, caster guid %u", getSpellInfo()->getId(), m_caster->getGuid());
    }
...
void Spell::handleHittedEffect(const uint64_t targetGuid, uint8_t effIndex, int32_t effDamage, bool reCheckTarget/* = false*/)
{
    ...
    {
        LOGGER.failure("Spell::handleHittedEffect : Unknown spell effect %u in spell id %u, index %u", effectId, getSpellInfo()->getId(), effIndex);
        return;
    }

    LOGGER.debug("Spell::handleHittedEffect : Spell effect %u, spell id %u, damage %d", effectId, getSpellInfo()->getId(), damage);
    ...
    if (p_caster != nullptr && getSpellInfo()->getRequiresSpellFocus() > 0)
    {
        ...
        for (const auto& itr : p_caster->getInRangeObjectsSet())
        {
            ...
            if (const auto obj = dynamic_cast<GameObject*>(itr))
            {
                ...
                {
                    LOGGER.debug("Spell::canCast : Found gameobject entry %u with invalid gameobject properties, spawn id %u", obj->getEntry(), obj->getGuidLow());
                    continue;
                }
...
SpellCastResult Spell::checkPower()
{
    ...
    {
        LOGGER.failure("Spell::checkPower : Unknown power type %u for spell id %u", getSpellInfo()->getPowerType(), getSpellInfo()->getId());
        ...
        switch (getSpellInfo()->getEffect(i))
        {
            ...
            case SPELL_EFFECT_CREATE_ITEM2:
                if (getSpellInfo()->getEffectItemType(i) != 0)
                {
                    ...
                    if (itemProperties == nullptr)
                    {
                        LOGGER.failure("Spell::checkItems: Spell entry %u has unknown item id (%u) in SPELL_EFFECT_CREATE_ITEM effect", getSpellInfo()->getId(), getSpellInfo()->getEffectItemType(i));
                        return SPELL_FAILED_ERROR;
                    }
            ...
            case SPELL_EFFECT_ENCHANT_ITEM:
                ...
                if (getSpellInfo()->getEffectItemType(i) != 0 && m_targets.getItemTarget() != 0 && vellumTarget)
                {
                    ...
                    if (itemProperties == nullptr)
                    {
                        LOGGER.failure("Spell::checkItems: Spell entry %u has unknown item id (%u) in SPELL_EFFECT_ENCHANT_ITEM effect", getSpellInfo()->getId(), getSpellInfo()->getEffectItemType(i));
                        return SPELL_FAILED_ERROR;
                    }
            ...
            case SPELL_EFFECT_ADD_SOCKET:
            {
                ...
                {
                    LOGGER.failure("Spell::checkItems: Spell entry %u has no valid enchantment (%u)", getSpellInfo()->getId(), getSpellInfo()->getEffectMiscValue(i));
                    return SPELL_FAILED_ERROR;
                }
...
SpellCastResult Spell::checkShapeshift(SpellInfo const* spellInfo, const uint32_t shapeshiftForm) const
{
    ...
    if (stanceMask > FORM_NORMAL)
    {
        ...
        {
            LOGGER.failure("Spell::checkShapeshift: Caster has unknown shapeshift form %u", shapeshiftForm);
            return SPELL_CAST_SUCCESS;
        }
...
void Spell::takePower()
{
    ...
    if (!getSpellInfo()->hasValidPowerType())
    {
        LOGGER.failure("Spell::takePower : Unknown power type %u for spell id %u", getSpellInfo()->getPowerType(), getSpellInfo()->getId());
        return;
    }
...
void Spell::_updateCasterPointers(Object* caster)
{
    ...
    switch (caster->getObjectTypeId())
    {
        ...
        case TYPEID_GAMEOBJECT:
            ...
            break;
        default:
            LOGGER.debug("Spell::_updateCasterPointers : Incompatible object type (type %u) for spell caster", caster->getObjectTypeId());
            break;
    }
}
        ...
        else
        {
            ...
            switch (wowGuid.getHigh())
            {
                ...
                case HighGuid::Corpse:
                    ...
                    break;
                default:
                    LOGGER.failure("Spell::_updateTargetPointers : Invalid object type for spell target (low guid %u) in spell %u", wowGuid.getGuidLowPart(), getSpellInfo()->getId());
                    break;
            }
        }
    }
