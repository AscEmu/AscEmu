...
void Set_Custom_BGR_one_buff_on_target(SpellEntry* sp)
{
    if (sp == nullptr)
    {
        Log.Error("Set_Custom_BGR_one_buff_on_target", "Something tried to call with an invalid spell pointer!");
        return;
    }
...
void Overwrite_procFlags(SpellEntry* sp)
{
    if (sp == nullptr)
    {
        Log.Error("Overwrite_procFlags", "Something tried to call with an invalid spell pointer!");
        return;
    }
