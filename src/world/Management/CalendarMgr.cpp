...
{
    logger.info("CalendarMgr : Start loading calendar_events");
    {
        ...
        if (!success)
        {
            logger.failure("Query failed: %s", loadCalendarEvents);
            return;
        }
        if (result)
        {
            ...
            do
            {
                ...
                logger.debug("Title %s loaded", calendarEvent->title.c_str()); // remove me ;-)
            ...
            logger.info("CalendarMgr : %u calendar events loaded from table calendar_events", count);
        }
    }

    logger.info("CalendarMgr : Start loading calendar_invites");
    {
        ...
        if (!success)
        {
            logger.failure("Query failed: %s", loadCalendarInvites);
            return;
        }
        if (result)
        {
            ...
            while (result->NextRow());
            delete result;
            logger.info("CalendarMgr : Loaded %u calendar invites", count);
        }
    }
}
