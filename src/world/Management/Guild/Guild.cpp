...
bool Guild::create(Player* pLeader, std::string const& name)
{
    ...
    LOGGER.debug("GUILD: creating guild %s for leader %s (%u)", name.c_str(), pLeader->getName().c_str(), WoWGuid::getGuidLowPartFromUInt64(m_leaderGuid));
...
void Guild::updateMemberData(Player* player, uint8_t dataid, uint32_t value)
{
    if (GuildMember* member = getMember(player->getGuid()))
    {
        switch (dataid)
        {
            ...
            default:
            {
                LOGGER.failure("Called with incorrect ID %u (val %u)", static_cast<uint32_t>(dataid), value);
                return;
            }
        }
...
void Guild::handleRoster(WorldSession* session)
{
    ...
    if (session)
    {
        LOGGER.debug("SMSG_GUILD_ROSTER %s", session->GetPlayer()->getName().c_str());
        session->SendPacket(&data);
    }
    else
    {
        LOGGER.debug("SMSG_GUILD_ROSTER [Broadcast]");
...
void Guild::handleQuery(WorldSession* session)
{
    ...
    //LOGGER.debug("SMSG_GUILD_QUERY_RESPONSE %s", session->GetPlayer()->getName().c_str());
}
...
void Guild::sendGuildRankInfo(WorldSession* session) const
{
    ...
    LOGGER.debug("SMSG_GUILD_RANK %s", session->GetPlayer()->getName().c_str());
}
#endif
...
void Guild::handleSetBankTabInfo(WorldSession* session, uint8_t tabId, std::string const& name, std::string const& icon)
{
    ...
    if (tab == nullptr)
    {
        LOGGER.failure("Player %s trying to change bank tab info from unexisting tab %d.", session->GetPlayer()->getName().c_str(), tabId);
        return;
    }
...
void Guild::handleSetRankInfo(WorldSession* session, uint8_t rankId, std::string const& name, uint32_t rights, uint32_t moneyPerDay, GuildBankRightsAndSlotsVec rightsAndSlots)
{
    ...
    {
        LOGGER.debug("Changed RankName to '%s', rights to 0x%08X", name.c_str(), rights);
...
void Guild::handleDisband(WorldSession* session)
{
    ...
    {
        disband();
        LOGGER.debug("Successfully Disbanded");
    }
}
...
void Guild::handleGuildPartyRequest(WorldSession* session)
{
    ...
    LOGGER.debug("SMSG_GUILD_PARTY_STATE_RESPONSE %s", session->GetPlayer()->getName().c_str());
}
...
void Guild::sendEventLog(WorldSession* session) const
{
    ...
    LOGGER.debug("SMSG_GUILD_EVENT_LOG_QUERY_RESULT %s", session->GetPlayer()->getName().c_str());
}
...
void Guild::sendNewsUpdate(WorldSession* session)
{
    ...
    LOGGER.debug("SMSG_GUILD_NEWS_UPDATE %s", session->GetPlayer()->getName().c_str());
}
#endif
...
void Guild::sendBankLog(WorldSession* session, uint8_t tabId) const
{
    if (tabId < _getPurchasedTabsSize() || tabId == MAX_GUILD_BANK_TABS)
    {
        ...
        LOGGER.debug("SMSG_GUILD_BANK_LOG_QUERY_RESULT %s TabId: %u", session->GetPlayer()->getName().c_str(), tabId);
    }
}
...
void Guild::sendPermissions(WorldSession* session) const
{
    ...
    LOGGER.debug("SMSG_GUILD_PERMISSIONS_QUERY_RESULTS %s Rank: %u", session->GetPlayer()->getName().c_str(), rankId);
}
...
void Guild::sendLoginInfo(WorldSession* session)
{
    ...
    LOGGER.debug("SMSG_GUILD_EVENT %s MOTD", session->GetPlayer()->getName().c_str());
    ...
    if (dbTabId < _getPurchasedTabsSize() || isMoneyTab)
    {
        ...
        if (pLog->canInsert())
        {
            ...
            if (GuildBankEventLogEntry::isMoneyEvent(eventType))
            {
                if (!isMoneyTab)
                {
                    LOGGER.failure("GuildBankEventLog ERROR: MoneyEvent(LogGuid: %u, Guild: %u) does not belong to money tab (%u), ignoring...", guid, m_id, dbTabId);
                    return false;
                }
            }
            else if (isMoneyTab)
            {
                LOGGER.failure("GuildBankEventLog ERROR: non-money event (LogGuid: %u, Guild: %u) belongs to money tab, ignoring...", guid, m_id);
                return false;
            }
...
void Guild::loadBankTabFromDB(Field* fields)
{
    ...
    if (tabId >= _getPurchasedTabsSize())
        LOGGER.failure("Invalid tab (tabId: %u) in guild bank, skipped.", tabId);
    else
...
bool Guild::loadBankItemFromDB(Field* fields)
{
    ...
    if (tabId >= _getPurchasedTabsSize())
    {
        LOGGER.failure("Invalid tab for item (GUID: %u) in guild bank, skipped.", fields[3].GetUInt32());
        return false;
    }
...
bool Guild::validate()
{
    ...
    {
        LOGGER.failure("Guild %u has invalid number of ranks, creating...", m_id);
        broken_ranks = true;
    }
    else
    {
        for (uint8_t rankId = 0; rankId < ranks; ++rankId)
        {
            ...
            {
                LOGGER.failure("Guild %u has invalid rank id %u, creating default set of ranks...", m_id, rankId);
                broken_ranks = true;
            }
            else
            {
...
void Guild::broadcastEvent(GuildEvents guildEvent, uint64_t guid, std::vector<std::string> vars) const
{
    ...
    LOGGER.debug("SMSG_GUILD_EVENT: %s (%u)", _GetGuildEventString(guildEvent).c_str(), guildEvent);
}
...
void Guild::sendGuildRanksUpdate(uint64_t setterGuid, uint64_t targetGuid, uint32_t rank)
{
    ...
    LOGGER.debug("SMSG_GUILD_RANKS_UPDATE target: %u, issuer: %u, rankId: %u",
...
void Guild::sendGuildReputationWeeklyCap(WorldSession* session, uint32_t reputation) const
{
    ...
    LOGGER.debug("SMSG_GUILD_REPUTATION_WEEKLY_CAP %s: Left: %u", session->GetPlayer()->getName().c_str(), cap);
}
...
void Guild::handleNewsSetSticky(WorldSession* session, uint32_t newsId, bool sticky)
{
    ...
    if (itr == logs->end())
    {
        LOGGER.debug("HandleNewsSetSticky: %s requested unknown newsId %u - Sticky: %u", session->GetPlayer()->getName().c_str(), newsId, sticky);
        return;
    }
    ...
    LOGGER.debug("HandleNewsSetSticky: %s chenged newsId %u sticky to %u", session->GetPlayer()->getName().c_str(), newsId, sticky);
...
void Guild::_sendBankContentUpdate(uint8_t tabId, SlotIds slots, bool sendAllSlots) const
{
    ...
    if (GuildBankTab const* guildBankTab = getBankTab(tabId))
    {
        ...
        LOGGER.debug("SMSG_GUILD_BANK_LIST");
    }
#endif
}
...
bool Guild::GuildMember::checkStats() const
{
    if (mLevel < 1)
    {
        LOGGER.failure("Player (GUID: %u) has a broken data in field `characters`.`level`, deleting him from guild!", WoWGuid::getGuidLowPartFromUInt64(mGuid));
        return false;
    }

    if (mClass < 1 || mClass >= 12)
    {
        LOGGER.failure("Player (GUID: %u) has a broken data in field `characters`.`class`, deleting him from guild!", WoWGuid::getGuidLowPartFromUInt64(mGuid));
        return false;
    }
...
bool Guild::GuildMember::loadGuildMembersFromDB(Field* fields, Field* fields2)
{
    ...
    if (!mZoneId)
    {
        LOGGER.failure("Player (GUID: %u) has broken zone-data", WoWGuid::getGuidLowPartFromUInt64(mGuid));
