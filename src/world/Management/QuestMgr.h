...
class SERVER_DECL QuestMgr
{
    ...
    public:
        ...
        void BuildQuestFailed(WorldPacket* data, uint32_t questid);
    ...
    private:
        ...
        std::unordered_map<uint32_t, std::list<QuestRelation*>* > m_npc_quests;
        std::unordered_map<uint32_t, std::list<QuestRelation*>* > m_obj_quests;
        std::unordered_map<uint32_t, std::list<QuestRelation*>* > m_itm_quests;
        QuestPOIMap m_QuestPOIMap;

        std::unordered_map<uint32_t, std::list<QuestAssociation*>* > m_quest_associations;
        inline std::unordered_map<uint32_t, std::list<QuestAssociation*>* >& GetQuestAssociationList()
        {return m_quest_associations;}

        std::unordered_map<uint32_t, uint32_t> m_ObjectLootQuestList;

        template <class T> void _AddQuest(uint32_t entryid, QuestProperties const* qst, uint8_t type);

        template <class T> std::unordered_map<uint32_t, std::list<QuestRelation*>* >& _GetList();
