...
void CBattlegroundManager::HandleBattlegroundJoin(WorldSession* m_session, WorldPacket& pck)
{
    ...
    LOGGER.info("BattlegroundManager : Player %u is now in battleground queue for instance %u", m_session->GetPlayer()->getGuidLow(), (srlPacket.instanceId + 1));
...
int CBattlegroundManager::CreateArenaType(int type, Group* group1, Group* group2)
{
    ...
    if (ar == nullptr)
    {
        LOGGER.failure("%s (%u): Couldn't create Arena Instance", __FILE__, __LINE__);
        m_queueLock.Release();
        m_instanceLock.Release();
        return -1;
    }
...
void CBattlegroundManager::EventQueueUpdate(bool forceStart)
{
    ...
    for (uint8 i = BATTLEGROUND_ARENA_2V2; i <= BATTLEGROUND_ARENA_5V5; ++i)
    {
        ...
        for (uint32 j = 0; j < static_cast<uint32>(m_queuedGroups[i].size()); j++)
        {
            ...
            if (itz == m_queuedGroups[i].end())
            {
                LOGGER.failure("Internal error at %s:%u", __FILE__, __LINE__);
                m_queueLock.Release();
                m_instanceLock.Release();
                return;
            }
...
void CBattlegroundManager::RemovePlayerFromQueues(Player* plr)
{
    ...
    while (itr != m_queuedPlayers[plr->m_bgQueueType][lgroup].end())
    {
        ...
        {
            LOGGER.debug("Removing player %u from queue instance %u type %u", plr->getGuidLow(), plr->m_bgQueueInstanceId, plr->m_bgQueueType);
    ...
    {
        LOGGER.debug("Player %u removed whilst in a group. Removing players group %u from queue", plr->getGuidLow(), group->GetID());
...
CBattleground* CBattlegroundManager::CreateInstance(uint32 Type, uint32 LevelGroup)
{
    if (bgMaps.find(Type) == bgMaps.end())
    {
        if (!isArena(Type))
        {
            LOGGER.failure("BattlegroundManager", "No map Id is registered for Battleground type %u", Type);
            return nullptr;
        }
    }
    ...
    if (isArena(Type))
    {
        ...
        if (arenaMapCount == 0)
        {
            LOGGER.failure("BattlegroundManager", "There are no Arenas registered. Cannot create Arena.");
            return nullptr;
        }
        ...
        if (mgr == nullptr)
        {
            LOGGER.failure("call failed for map %u, type %u, level group %u", mapid, Type, LevelGroup);
        ...
        LOGGER.info("BattlegroundManager : Created arena battleground type %u for level group %u on map %u.", Type, LevelGroup, mapid);
    ...
    if (cfunc == nullptr)
    {
        LOGGER.failure("Could not find CreateBattlegroundFunc pointer for type %u level group %u", Type, LevelGroup);
        return nullptr;
    }
    ...
    if (mgr == nullptr)
    {
        LOGGER.failure("call failed for map %u, type %u, level group %u", bgMaps[Type], Type, LevelGroup);
        return nullptr;      // Shouldn't happen
    }
    ...
    mgr->m_battleground = bg;
    sEventMgr.AddEvent(bg, &CBattleground::EventCreate, EVENT_BATTLEGROUND_QUEUE_UPDATE, 1, 1, 0);
    LOGGER.info("BattlegroundManager : Created battleground type %u for level group %u.", Type, LevelGroup);
...
void CBattlegroundManager::DeleteBattleground(CBattleground* bg)
{
    ...
    //LOGGER.info("Deleting battleground from queue %u, instance %u", bg->GetType(), bg->GetId());
    delete bg;
}
...
void CBattlegroundManager::HandleArenaJoin(WorldSession* m_session, uint32 BattlegroundType, uint8 as_group, uint8 rated_match)
{
    ...
    if (as_group)
    {
        ...
        m_queueLock.Release();
        LOGGER.info("BattlegroundMgr : Group %u is now in battleground queue for arena type %u", pGroup->GetID(), BattlegroundType);
    ...
    LOGGER.info("BattlegroundMgr : Player %u is now in battleground queue for {Arena %u}", m_session->GetPlayer()->getGuidLow(), BattlegroundType);
