...
uint32_t QuestMgr::PlayerMeetsReqs(Player* plr, QuestProperties const* qst, bool skiplevelcheck)
{
    ...
    if (qst->required_rep_faction && qst->required_rep_value)
        if (plr->GetStanding(qst->required_rep_faction) < (int32_t)qst->required_rep_value)
            return QuestStatus::NotAvailable;
    ...
    {
        std::set<uint32_t>::iterator iter = qst->quest_list.begin();
...
uint32_t QuestMgr::CalcStatus(Object* quest_giver, Player* plr)
{
    ...
    {
        uint32_t tmp_status = CalcQuestStatus(quest_giver, plr, *itr); // save a call
        if (tmp_status > status)
            status = tmp_status;
    }
    ...
    for (it = st; it != ed; ++it)
    {
        ...
        if (status >= QuestStatus::AvailableChat)
        {
            if (tmp_map.find((*it)->qst->id) == tmp_map.end())
            {
                ...
                *data << uint32_t(0);*/
                ...
                switch (status)
                {
                    ...
                    case QuestStatus::AvailableChat:
                        *data << uint32_t(QuestStatus::Available);
                        break;
                ...
                *data << uint8_t(0);   // According to MANGOS: "changes icon: blue question or yellow exclamation"
...
bool QuestMgr::OnGameObjectActivate(Player* plr, GameObject* go)
{
    ...
    for (uint8_t i = 0; i < 25; ++i)
    {
        ...
        if (qle != NULL)
        {
            ...
            {
                if (qst->required_mob_or_go[j] == static_cast<int32_t>(entry) && qst->required_mobtype[j] == QUEST_MOB_TYPE_GAMEOBJECT && qle->m_mobcount[j] < qst->required_mob_or_go_count[j])
                {
                    // add another kill.
                    // (auto-dirty's it)
        ...
        for (uint8_t i = 0; i < 25; ++i)
        {
            ...
            if (quest_log_entry)
            {
                ...
                for (uint8_t j = 0; j < 4; ++j)
                {
                    ...
                    if (qst->required_mob_or_go[j] == static_cast<int32_t>(entry) && qst->required_mobtype[j] == QUEST_MOB_TYPE_CREATURE && quest_log_entry->m_mobcount[j] < qst->required_mob_or_go_count[j])
                    {
    ...
    if (IsGroupKill)
    {
        ...
        if (plr->InGroup())
        {
            if (Group* pGroup = plr->GetGroup())
            {
                ...
                pGroup->Lock();
                for (uint32_t k = 0; k < pGroup->GetSubGroupCount(); k++)
                {
...
void QuestMgr::OnPlayerCast(Player* plr, uint32_t spellid, uint64_t & victimguid)
{
    ...
    uint32_t entry = (victim) ? victim->getEntry() : 0;
    ...
    for (uint8_t i = 0; i < 25; ++i)
    {
        ...
        if (quest_log_entry)
        {
            ...
            for (uint8_t j = 0; j < 4; ++j)
            {
                ...
                {
                    if (victim && quest->required_mob_or_go[j] == static_cast<int32_t>(entry) && quest->required_spell[j] == spellid && (quest_log_entry->m_mobcount[j] < quest->required_mob_or_go_count[j] || quest_log_entry->m_mobcount[j] == 0) && !quest_log_entry->IsUnitAffected(victim))
                    {
...
void QuestMgr::GiveQuestRewardReputation(Player* plr, QuestProperties const* qst, Object* qst_giver)
{
    ...
    for (uint8_t z = 0; z < 6; ++z)
    {
        ...
        int32_t amt = float2int32(GenerateQuestXP(plr, qst) * 0.1f);      // guess
        ...
        if (qst->reward_replimit)
            if (plr->GetStanding(fact) >= (int32_t)qst->reward_replimit)
                continue;
...
{
    std::unordered_map<uint32_t, QuestRelationList* > &olist = _GetList<GameObject>();
    std::unordered_map<uint32_t, QuestRelationList* >::iterator itr = olist.find(entryid);
    return (itr == olist.end()) ? 0 : itr->second;
}

QuestRelationList* QuestMgr::GetCreatureQuestList(uint32_t entryid)
{
    std::unordered_map<uint32_t, std::list<QuestRelation*>* > &olist = _GetList<Creature>();
    std::unordered_map<uint32_t, QuestRelationList* >::iterator itr = olist.find(entryid);
    return (itr == olist.end()) ? 0 : itr->second;
}
...
void QuestMgr::BuildQuestFailed(WorldPacket* data, uint32_t questid)
{
...
bool QuestMgr::OnActivateQuestGiver(Object* qst_giver, Player* plr)
{
    ...
    uint32_t questCount = sQuestMgr.ActiveQuestsCount(qst_giver, plr);
    ...
    else if (questCount == 1)
    {
        ...
        uint32_t status = sQuestMgr.CalcStatus(qst_giver, plr);
...
{
    std::unordered_map<uint32_t, QuestProperties*>::iterator itr1;
    std::unordered_map<uint32_t, std::list<QuestRelation*>* >::iterator itr2;
    std::list<QuestRelation*>::iterator itr3;
    std::unordered_map<uint32_t, std::list<QuestAssociation*>* >::iterator itr4;
...
void QuestMgr::LoadExtraQuestStuff()
{
    ...
    if (result != NULL)
    {
        ...
        do
        {
            ...
            uint32_t questId = fields[0].GetUInt32();
            uint32_t poiId = fields[1].GetUInt32();
            int32_t  objIndex = fields[2].GetInt32();
            uint32_t mapId = fields[3].GetUInt32();
            uint32_t mapAreaId = fields[4].GetUInt32();
            uint32_t floorId = fields[5].GetUInt32();
            uint32_t unk3 = fields[6].GetUInt32();
            uint32_t unk4 = fields[7].GetUInt32();
        ...
        if (points != NULL)
        {
            ...
            do
            {
                ...
                uint32_t questId = pointFields[0].GetUInt32();
                uint32_t poiId = pointFields[1].GetUInt32();
                int32_t  x = pointFields[2].GetInt32();
                int32_t  y = pointFields[3].GetInt32();
...
{
    std::unordered_map<uint32_t, std::list<QuestAssociation*>* > &associationList = GetQuestAssociationList();
    ...
    if (associationList.find(itemId) == associationList.end())
    {
        ...
        associationList.insert(std::unordered_map<uint32_t, std::list<QuestAssociation*>* >::value_type(itemId, tempList));
    }
    else
    {
...
{
    std::unordered_map<uint32_t, QuestAssociationList* > &associationList = GetQuestAssociationList();
    std::unordered_map<uint32_t, QuestAssociationList* >::iterator itr = associationList.find(itemId);
    ...
    for (i = 0; i < 25; ++i)
    {
        if ((qle = plr->GetQuestLogInSlot(i)) != 0)
        {
            ...
            for (j = 0; j < 4; ++j)
            {
                ...
                {
                    if (victim && qst->required_mob_or_go[j] == static_cast<int32_t>(entry) && qst->required_emote[j] == emoteid && (qle->m_mobcount[j] < qst->required_mob_or_go_count[j] || qle->m_mobcount[j] == 0) && !qle->IsUnitAffected(victim))
                    {
