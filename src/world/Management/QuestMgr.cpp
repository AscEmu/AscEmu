...
uint32 QuestMgr::CalcStatus(Object* quest_giver, Player* plr)
{
    ...
    if (!bValid)
    {
        ...
        //LOGGER.debug("QUESTS: Warning, invalid NPC " I64FMT " specified for CalcStatus. TypeId: %d.", quest_giver->getGuid(), quest_giver->getObjectTypeId());
        return status;
    }
...
void QuestMgr::SetGameObjectLootQuest(uint32 GO_Entry, uint32 Item_Entry)
{
    ...
    /*if (QuestID == 0)
        LOGGER.debug("QuestMgr : No corresponding quest was found for loot_gameobjects entryid %u quest item %d", GO_Entry, Item_Entry);*/
}
