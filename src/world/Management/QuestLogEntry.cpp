...
void QuestLogEntry::setSlot(uint8_t slot)
{
    ...
    {
        logger.failure("%u is not a valid questlog slot!", uint32_t(slot));
        return;
    }
...
uint32_t QuestLogEntry::getMobCountByIndex(uint8_t index) const
{
    if (index >= 4)
    {
        logger.failure("%u is not a valid index for questlog mob count!", uint32_t(index));
        return 0;
    }
...
uint32_t QuestLogEntry::getExploredAreaByIndex(uint8_t index) const
{
    if (index >= 4)
    {
        logger.failure("%u is not a valid index for questlog explore areas!", uint32_t(index));
        return 0;
    }
