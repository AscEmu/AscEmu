    ...
    if (achievementResult)
    {
        do
        {
            ...
            if (m_completedAchievements[id] == 0)
                ...
                LOGGER.failure("Duplicate completed achievement %u for player %u, skipping", id, (uint32_t)m_player->getGuid());
        }
    ...
    if (criteriaResult)
    {
        do
        {
            ...
            if (m_criteriaProgress[progress_id] == nullptr)
            {
                ...
                LOGGER.failure("Duplicate criteria progress %u for player %u, skipping", progress_id, (uint32_t)m_player->getGuid());
...
void AchievementMgr::GiveAchievementReward(DBC::Structures::AchievementEntry const* entry)
{
    ...
    if (Reward->sender)
    {
        ...
        {
            LOGGER.failure("can not create sender for achievement %u", entry);
            return;
        }
        ...
        {
            LOGGER.failure("Can not create item for message! (nullptr)");
            return;
        }
    }
}
...
bool AchievementMgr::GMCompleteCriteria(WorldSession* gmSession, int32_t criteriaID)
{
    if (criteriaID == -1)
    {
        ...
        for (uint32_t i = 0, j = 0; j < nr; ++i)
        {
            ...
            if (crt == nullptr)
            {
                LOGGER.failure("Achievement Criteria %lu entry not found.", i);
                continue;
            }
            ++j;
            if (crt->raw.field4)
...
bool AchievementMgr::UpdateAchievementCriteria(Player* player, int32_t criteriaID, uint32_t count)
{
    ...
    if (!criteria)
    {
        LOGGER.debug("Achievement ID %u is Invalid", criteriaID);
        return false;
    }
    if (IsCompletedCriteria(criteria))
    {
        LOGGER.debug("Achievement criteria %lu already completed.", criteriaID);
        return false;
    }
    auto* achievement = sAchievementStore.LookupEntry(criteria->referredAchievement);
    if (!achievement)
    {
        // achievement not found
        LOGGER.debug("Referred achievement (%lu) entry not found.", criteria->referredAchievement);
        return false;
    }
    if (achievement->flags & ACHIEVEMENT_FLAG_COUNTER)
    {
        // can't complete this type of achivement (counter)
        LOGGER.debug("AchievementMgr Referred achievement (%u) |Hachievement:%u:" I64FMT ":0:0:0:-1:0:0:0:0|h[%s]|h is a counter and cannot be completed.",
