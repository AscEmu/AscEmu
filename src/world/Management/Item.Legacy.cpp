...
void Item::ApplyEnchantmentBonus(uint32 Slot, bool Apply)
{
    ...
    {
        logger.failure("Item::ApplyEnchantmentBonus visual out of range! Tried to address UInt32 field %i !!!", ItemSlot);
    }
    ...
    for (uint32 c = 0; c < 3; c++)
    {
        if (Entry->type[c])
        {
            ...
            switch (Entry->type[c])
            {
                ...
                default:
                    logger.failure("Unknown enchantment type: %u (%u)", Entry->type[c], Entry->Id);
                break;
            }
        }
    }
...
uint32 Item::RepairItemCost()
{
    ...
    {
        logger.failure("Repair: Unknown item level (%u)", durability_costs);
        return 0;
    }
    ...
    {
        logger.failure("Repair: Unknown item quality (%u)", durability_quality);
        return 0;
    }
