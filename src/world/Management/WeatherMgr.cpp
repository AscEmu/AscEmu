...
void WeatherInfo::BuildUp()
{
    ...
    if (m_currentDensity >= 0.50f)
    {
        ...
        //        LOGGER.debug("Weather starting random for zone:%d type:%d new interval:%d ms",m_zoneId,m_currentEffect,(uint32)(m_totalTime/ceil(m_maxDensity/WEATHER_DENSITY_UPDATE)*4));
    }
    else
    {
        m_currentDensity += WEATHER_DENSITY_UPDATE;
        //        LOGGER.debug("Weather increased for zone:%d type:%d density:%f",m_zoneId,m_currentEffect,m_currentDensity);
        SendUpdate();
    }
}
...
void WeatherInfo::Update()
{
    ...
    SendUpdate();
    //    LOGGER.debug("Weather Updated,zoneId:%d type:%d density:%f", m_zoneId, m_currentEffect, m_currentDensity);
}
