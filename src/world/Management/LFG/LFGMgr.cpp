...
void LfgMgr::LoadRewards()
{
    ...
    do
    {
        ...
        {
            LOGGER.debug("Dungeon %u specified in table `lfg_dungeon_rewards` does not exist!", dungeonId);
            continue;
        }
#endif
    ...
    else
    {
        ...
        else if (player->GetMapId() != uint32(dungeon->map))  // Do not teleport players in dungeon to the entrance
        {
            ...
            if (!mapid)
            {
                ...
                {
                    LOGGER.debug("Failed to teleport %u: No areatrigger found for map: %u difficulty: %u", player->getGuid(), dungeon->map, dungeon->difficulty);
                    error = LFG_TELEPORTERROR_INVALID_LOCATION;
                }
                else
                {
...
void LfgMgr::RewardDungeonDoneFor(const uint32 dungeonId, Player* player)
{
    ...
    {
        LOGGER.debug("%u is not in a group or not a LFGGroup. Ignoring", player->getGuid());
        return;
    }
    ...
    {
        LOGGER.debug("%u Finished dungeon %u but group queued for %u. Ignoring", guid, dungeonId, gDungeonId);
        return;
    }

    if (GetState(guid) == LFG_STATE_FINISHED_DUNGEON)
    {
        LOGGER.debug("%u Already rewarded player. Ignoring", guid);
        return;
    }
    ...
    {
        LOGGER.debug("%u dungeon %u is not random nor seasonal", guid, rDungeonId);
        return;
    }
    ...
    if (!player->hasQuestInFinishedDailies(qReward->id) || !player->HasFinishedQuest(qReward->id))
    {
        ...
        for (uint8 i = 0; i < 4; ++i)
        {
            if (qReward->reward_item[i])
            {
                ...
                if (!proto)
                {
                    LOGGER.failure("Invalid item prototype in quest reward! ID %d, quest %d", qReward->reward_item[i], qReward->id);
                }
                else
                {
    ...
    LOGGER.debug("%u done dungeon %u, previously done.", player->getGuid(), GetDungeon(gguid));
