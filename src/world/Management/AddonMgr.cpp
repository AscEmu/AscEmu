...
bool AddonMgr::IsAddonBanned(std::string name, uint64 crc)
{
    ...
    if (itr != mKnownAddons.end())
    {
        ...
        {
            LOGGER.debug("Addon %s is banned.", name.c_str());
            return true;
        }
    }
    else
    {
        ...
        LOGGER.debug("Discovered new addon %s sent by client.", name.c_str());
...
void AddonMgr::SendAddonInfoPacket(WorldPacket* source, uint32 /*pos*/, WorldSession* m_session)
{
    ...
    {
        LOGGER.debug("Warning: Incomplete auth session sent.");
        return;
    }
    ...
    LOGGER.info("Decompression of addon section of CMSG_AUTH_SESSION succeeded.");
...
void AddonMgr::LoadFromDB()
{
    ...
    if (!success)
    {
        LOGGER.failure("Query failed: %s", loadClientAddons);
        return;
    }
    if (!result)
    {
        LOGGER.info("AddonMgr : No defined ClientAddons");
        return;
    }
...
{
    LOGGER.info("AddonMgr: Saving any new addons discovered in this session to database.");
    ...
    for (itr = mKnownAddons.begin(); itr != mKnownAddons.end(); ++itr)
    {
        if (itr->second->isNew)
        {
            LOGGER.info("Saving new addon %s", itr->second->name.c_str());
            std::stringstream ss;
