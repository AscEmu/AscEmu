...
WorldPacket* WorldSession::BuildQuestQueryResponse(QuestProperties const* pQuest)
{
    ...
    std::string QuestGiverTargetName = pQuest->GetQuestGiverPortraitUnk();
    std::string QuestTurnTextWindow = pQuest->GetQuestTurnInPortraitText();
    std::string QuestTurnTargetName = pQuest->GetQuestTurnInPortraitUnk();

    for (uint8 i = 0; i < QUEST_OBJECTIVES_COUNT; ++i)
        ObjectiveText[i] = pQuest->ObjectiveText[i];

    WorldPacket data(SMSG_QUEST_QUERY_RESPONSE, 248);
    ...
    if (pQuest->HasFlag(QUEST_FLAGS_HIDDEN_REWARDS))
    {
        for (uint8 iI = 0; iI < QUEST_REWARDS_COUNT; ++iI)
            data << uint32(0) << uint32(0);
        for (uint8 iI = 0; iI < QUEST_REWARD_CHOICES_COUNT; ++iI)
            data << uint32(0) << uint32(0);
    }
    else
    {
    ...
    for (uint8 iI = 0; iI < QUEST_OBJECTIVES_COUNT; ++iI)
    {
    ...
    for (uint8 iI = 0; iI < QUEST_ITEM_OBJECTIVES_COUNT; ++iI)
    {
    ...
    for (uint8 iI = 0; iI < 4; ++iI)                               // 4.0.0 currency reward id and count
    {
        data << uint32(pQuest->RewCurrencyId[iI]);
        data << uint32(pQuest->RewCurrencyCount[iI]);
    }

    for (uint8 iI = 0; iI < 4; ++iI)                               // 4.0.0 currency required id and count
    {
