...
void Arcemu::Gossip::ClassTrainer::OnHello(Object* pObject, Player* Plr)
{
    ...
    if (traininfo != NULL)    //Seems to happen
    {
        ...
        else
        {
            ...
            //talent reset option.
            if (trainer->getLevel() > TrainerTalentResetMinLevel && Plr->getLevel() > worldConfig.optional.minTalentResetLevel && trainer->GetTrainer()->RequiredClass == playerclass)
            {
                menu.AddItem(GOSSIP_ICON_CHAT, Plr->GetSession()->LocalizedGossipOption(CLASSTRAINER_TALENTRESET), 2);
                //dual speciliazation option.
                if (Plr->getLevel() >= worldConfig.optional.minDualSpecLevel && Plr->m_talentSpecsCount < 2)
