...
uint32_t InstanceMgr::PreTeleport(uint32_t mapid, Player* plr, uint32_t instanceid)
{
    ...
    plr->SetInstanceID(in->m_instanceId);
    LOGGER.debug("Creating instance for player %u and group %u on map %u. (%u)", in->m_creatorGuid, in->m_creatorGroup, in->m_mapId, in->m_instanceId);
...
MapMgr* InstanceMgr::_CreateInstance(uint32_t mapid, uint32_t instanceid)
{
    ...
    LOGGER.info("InstanceMgr : Creating continent %s.", m_maps[mapid]->GetMapName().c_str());
...
MapMgr* InstanceMgr::_CreateInstance(Instance* in)
{
    ...
    LOGGER.info("InstanceMgr : Creating saved instance %u (%s)", in->m_instanceId, m_maps[in->m_mapId]->GetMapName().c_str());
...
void InstanceMgr::DeleteBattlegroundInstance(uint32_t mapid, uint32_t instanceid)
{
    ...
    {
        LOGGER.failure("Could not delete battleground instance!");
        m_mapLock.Release();
        return;
    }
