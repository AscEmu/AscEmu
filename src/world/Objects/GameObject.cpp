    ...
    if (gameobject_properties->button.linked_trap_id != 0)
    {
        ...
        if (gameobject_info != nullptr)
        {
            if (gameobject_info->trap.spell_id != 0)
                spell = sSpellCustomizations.GetServersideSpell(gameobject_info->trap.spell_id);
        }
    }
}
...
void GameObject_Trap::InitAI()
{
    ...
    spell = sSpellCustomizations.GetServersideSpell(gameobject_properties->trap.spell_id);
...
void GameObject_SpellCaster::InitAI()
{
    ...
    spell = sSpellCustomizations.GetServersideSpell(gameobject_properties->spell_caster.spell_id);
    if (spell == nullptr)
