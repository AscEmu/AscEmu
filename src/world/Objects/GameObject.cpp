...
bool GameObject::CreateFromProto(uint32 entry, uint32 mapid, float x, float y, float z, float ang, float r0, float r1, float r2, float r3, uint32 overrides)
{
    ...
    {
        logger.failure("Something tried to create a GameObject with invalid entry %u", entry);
        return false;
    }
...
void GameObject::CastSpell(uint64 TargetGUID, uint32 SpellID)
{
    ...
    if (sp == nullptr)
    {
        logger.failure("GameObject %u tried to cast a non-existing Spell %u.", gameobject_properties->entry, SpellID);
        return;
    }
...
{
    logger.failure("Player uses Button.");
    if (getState() == GO_STATE_CLOSED)
    {
        Open();
...
{
    logger.failure("Player uses Chair.");
...
void GameObject_SpellFocus::SpawnLinkedTrap()
{
    ...
    if (go == nullptr)
    {
        logger.failure("Failed to create linked trap (entry: %u) for GameObject %u ( %s ). Missing GOProperties!", trapid, gameobject_properties->entry, gameobject_properties->name.c_str());
        return;
    }

    if (!go->CreateFromProto(trapid, m_mapId, m_position.x, m_position.y, m_position.z, m_position.o))
    {
        logger.failure("Failed CreateFromProto for linked trap of GameObject %u ( %s ).", gameobject_properties->entry, gameobject_properties->name.c_str());
        return;
    }
...
{
    logger.failure("Player uses Goober.");
    if (getState() == GO_STATE_CLOSED)
    {
        Open();
...
void GameObject_FishingNode::onUse(Player* player)
{
    ...
    if (success)
    {
        ...
        if (entry == nullptr)
        {
            logger.failure("ERROR: Fishing zone information for zone %u not found!", zone);
            EndFishing(true);
            return;
        }
...
void GameObject_SpellCaster::InitAI()
{
    ...
    if (spell == nullptr)
        logger.failure("GameObject %u ( %s ) has a nonexistant spellID in the database.", gameobject_properties->entry, gameobject_properties->name.c_str());
}
