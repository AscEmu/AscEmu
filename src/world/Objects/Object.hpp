...
public:
    ...
    void PushToWorld(WorldMap* map);
    void AddToWorld(WorldMap* map);
    void RemoveFromWorld(bool freeGuid = false);
...
public:
    ...
    bool isValidAttackableTarget(Object const* target, SpellInfo const* bySpell = nullptr, bool requireVisibility = true) const;
    ...
    uint32_t GetUIdFromGUID() const { return (getGuidLow() & LOWGUID_ENTRY_MASK); }
        ...
        WorldMap* getLifecycleMap() const { return m_lifecycleMap; }
        void setLifecycleMap(WorldMap* map) { m_lifecycleMap = map; }
        ...
        GameObject* summonGameObject(uint32_t entryID, LocationVector pos, QuaternionData const& rot, uint32_t durationMs = 0, GOSummonType summonType = GO_SUMMON_TIMED_OR_CORPSE_DESPAWN);
    ...
    protected:
        ...
        WorldMap* m_lifecycleMap = nullptr;
