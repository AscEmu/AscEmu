    ...
    if (result)
    {
        do
        {
            ...
            if (seo_SpellId)
            {
                ...
                {
                    Log.DebugFlag(LF_DB_TABLES, "ObjectMgr : Tried to load a spell effect override for a nonexistant spell: %u", seo_SpellId);
                }
            }
...
void ObjectMgr::LoadSpellRequired()
{
    ...
    if (!result)
    {
        Log.DebugFlag(LF_DB_TABLES, "ObjectMgr : Loaded 0 spell required records. DB table `spell_required` is empty.");
        return;
    }
    ...
    do
    {
        ...
        if (!spell)
        {
            Log.DebugFlag(LF_DB_TABLES, "ObjectMgr : spell_id %u in `spell_required` table is not found in dbcs, skipped", spell_id);
            continue;
        }
        ...
        if (!req_spell)
        {
            Log.DebugFlag(LF_DB_TABLES, "ObjectMgr : req_spell %u in `spell_required` table is not found in dbcs, skipped", spell_req);
            continue;
        }

        if (IsSpellRequiringSpell(spell_id, spell_req))
        {
            Log.DebugFlag(LF_DB_TABLES, "ObjectMgr : duplicated entry of req_spell %u and spell_id %u in `spell_required`, skipped", spell_req, spell_id);
            continue;
        }
        ...
        else
        {
            do
            {
                ...
                for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
                {
                    ...
                    {
                        Log.DebugFlag(LF_DB_TABLES, "LoadTrainers : Table `trainer_spels` has spell %u for trainer entry %u with learn effect which has incorrect target type, ignoring learn effect!", spell, entry);
                        continue;
                    }
    ...
    for (EventScriptMaps::const_iterator itr = EventScript.first; itr != EventScript.second; ++itr)
    {
        if (itr->second.scripttype == static_cast<uint8>(EasyScriptTypes::SCRIPT_TYPE_SPELL_EFFECT) || itr->second.scripttype == static_cast<uint8>(EasyScriptTypes::SCRIPT_TYPE_DUMMY))
        {
            ...
            case static_cast<uint8>(ScriptCommands::SCRIPT_COMMAND_KILL_CREDIT):
            {
                ...
                if (pQuest != nullptr)
                {
                    if (pQuest->GetMobCount(itr->second.data_5) < pQuest->GetQuest()->required_mob_or_go_count[itr->second.data_5])
                    {
...
void ObjectMgr::LoadQuestLoot(uint32 GO_Entry, uint32 Item_Entry)
{
    ...
    /*for (auto itr = sMySQLStore._questPropertiesStore.begin(); itr != sMySQLStore._questPropertiesStore.end(); ++itr)
    {
        ...
        {
            if (itr->second.required_item[i] == Item_Entry)
            {
                QuestID = itr->second.id;
