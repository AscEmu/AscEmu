...
bool Object::SetPosition(const LocationVector & v, [[maybe_unused]]bool allowPorting /* = false */)
{
    ...
    {
        m_position.z = 500;
        LOGGER.failure("setPosition: fell through map; height ported");
...
bool Object::inArc(float Position1X, float Position1Y, float FOV, float Orientation, float Position2X, float Position2Y)
{
    ...
    //LOGGER.debug("Orientation: %f Angle: %f LeftBorder: %f RightBorder %f",Orientation,angle,lborder,rborder);
...
void MovementInfo::readMovementInfo(ByteBuffer& data, uint16_t opcode)
{
    ...
    LOGGER.debug("guid: %u, flags: %u, flags2: %u, updatetime: %u, position: (%f, %f, %f, %f)",
    ...
    if (hasMovementFlag(MOVEFLAG_TRANSPORT))
    {
        ...
        LOGGER.debug("tguid: %u, tposition: (%f, %f, %f, %f)", transport_guid, transport_position.x, transport_position.y, transport_position.z, transport_position.o);
    }
