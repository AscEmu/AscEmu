...
bool ChatHandler::HandleKickByNameCommand(const char* args, WorldSession* m_session)
{
    ...
    if (player_target != nullptr)
    {
        ...
        if (sWorld.settings.gmSettings.worldAnnounceOnKickPlayer)
        {
            char msg[200];
...
bool ChatHandler::HandleAnnounceCommand(const char* args, WorldSession* m_session)
{
    ...
    colored_text += "[";
    colored_text += sWorld.settings.announceSettings.announceTag;
    colored_text += "]";
    colored_text += sWorld.ann_gmtagcolor;

    if (sWorld.settings.announceSettings.enableGmAdminTag)
    {
    ...
    if (sWorld.settings.announceSettings.showNameInAnnounce)
    {
    ...
    else if (!sWorld.settings.announceSettings.showNameInAnnounce)
    {
...
bool ChatHandler::HandleWAnnounceCommand(const char* args, WorldSession* m_session)
{
    ...
    colored_widescreen_text += sWorld.settings.announceSettings.announceTag;
    colored_widescreen_text += "]";
    ...
    if (sWorld.settings.announceSettings.showNameInWAnnounce)
    {
    ...
    else if (!sWorld.settings.announceSettings.showNameInWAnnounce)
    {
