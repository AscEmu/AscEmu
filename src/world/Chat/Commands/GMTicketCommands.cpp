...
bool ChatHandler::HandleGMTicketListCommand(const char* /*args*/, WorldSession* m_session)
{
    ...
    for (GmTicketList::iterator itr = sObjectMgr.GM_TicketList.begin(); itr != sObjectMgr.GM_TicketList.end(); ++itr)
    {
        ...
        //        Player* aplr = ((*itr)->assignedToPlayer == 0 ? NULL : sObjectMgr.GetPlayer((uint32_t)(*itr)->assignedToPlayer));
...
bool ChatHandler::HandleGMTicketListCommand(const char* args, WorldSession* m_session)
{
    ...
    for (GmTicketList::iterator itr = sObjectMgr.GM_TicketList.begin(); itr != sObjectMgr.GM_TicketList.end(); itr++)
    {
        ...
        if ((*itr)->assignedToPlayer != 0)
        {
            ...
            if (aplr == NULL)
                aplri = sObjectMgr.GetPlayerInfo((uint32_t)(*itr)->assignedToPlayer);
        }
