        ...
        if (sub_command.empty())
        {
            ...
            while (p->Name != 0)
            {
                ...
                if (!curr_table.compare(main_command))
                {
                    ...
                    logger.debug("Changing command level of .`%s` to %c.", main_command.c_str(), level[0]);
                    break;
                }
                ++p;
            }
                ...
                if (!curr_table.compare(main_command))
                {
                    ...
                    while (p2->Name != 0)
                    {
                        ...
                        if (!curr_subcommand.compare(sub_command))
                        {
                            ...
                            logger.debug("Changing command level of .`%s %s` to %c.", main_command.c_str(), sub_command.c_str(), level[0]);
                            break;
                        }
                        ++p2;
                    }
                    ...
                    if (!curr_subcommand.compare(sub_command))
                    {
                        ...
                        while (p3->Name != 0)
                        {
                            ...
                            if (!curr_sec_subcommand.compare(sec_sub_command))
                            {
                                ...
                                logger.debug("Changing command level of .`%s %s %s` to %c.", main_command.c_str(), sub_command.c_str(), sec_sub_command.c_str(), level[0]);
                                break;
                            }
                            ++p3;
                        }
