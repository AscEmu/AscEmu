...
public:
    ...
    {
        Creature* ct = mTarget->getWorldMap()->createAndSpawnCreature(22911, 3279.67f, 4640.77f, 216.526f, 1.3516f);
        if (ct != nullptr)
...
public:
    void OnQuestStart(Player* mTarget, QuestLogEntry* /*qLogEntry*/) override
    {
        Creature* ct = mTarget->getWorldMap()->createAndSpawnCreature(20216, 2687.46f, 5541.14f, -1.93669f, 3.52847f);
        if (ct != nullptr)
            ct->Despawn(2 * 60 * 1000, 0);
    }
};
...
public:
    void OnQuestStart(Player* mTarget, QuestLogEntry* /*qLogEntry*/) override
    {
        Creature* ct = mTarget->getWorldMap()->createAndSpawnCreature(11980, -4177.39f, 376.289f, 117.78f, 2.7381f);
        if (ct != nullptr)
            ct->Despawn(2 * 60 * 1000, 0);
    }
};
