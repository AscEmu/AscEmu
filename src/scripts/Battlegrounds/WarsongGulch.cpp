...
void WarsongGulch::HookOnAreaTrigger(Player* plr, uint32 id)
{
    ...
    if (buffslot >= 0)
    {
        ...
        {
            // apply the buff
            OLD_SpellEntry* sp = dbcSpell.LookupEntry(m_buffs[buffslot]->GetGameObjectProperties()->raw.parameter_3);
...
void WarsongGulch::HookFlagDrop(Player* plr, GameObject* obj)
{
    ...
    OLD_SpellEntry* pSp = dbcSpell.LookupEntry(23333 + (plr->GetTeam() * 2));
