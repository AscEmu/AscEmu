...
void WarsongGulch::HookOnAreaTrigger(Player* plr, uint32_t id)
{
    ...
    if (((id == AREATRIGGER_WSG_A_SPAWN && plr->isTeamAlliance()) || (id == AREATRIGGER_WSG_H_SPAWN && plr->isTeamHorde())) && (plr->hasBgFlag() && m_flagHolders[plr->getTeam()] == plr->GetNewGUID()))
    {
        ...
        if (distance > 50.0f)
        {
            ...
            sCheatLog.writefromsession(plr->getSession(), "{} attempted to capture the Warsong Gulch flag from more than 50 yards away (distance: {:.2f} yards)", plr->getName(), plr->CalcDistance(915.367f, 1433.78f, 346.089f));
...
void WarsongGulch::HookFlagStand(Player* plr, GameObject* obj)
{
    if (!m_hasStarted)
    {
        sCheatLog.writefromsession(plr->getSession(), "{} attempted to pick up the Warsong Gulch flag before the battleground started (ID: {}).", plr->getName(), this->m_id);
        sendChatMessage(CHAT_MSG_BG_EVENT_NEUTRAL, plr->getGuid(), "{} attempted to pick up the Warsong Gulch flag before the battleground started and will be removed for cheating.", plr->getName());
