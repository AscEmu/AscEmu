...
void NaxxramasWorshipperAI::OnDied(Unit* pKiller)
{
    ...
    if (mGrandWidow != NULL)   //&& !IsHeroic())
    {
        ...
        {
            for (auto Iter = mGrandWidow->mWorshippers.begin(); Iter != mGrandWidow->mWorshippers.end(); ++Iter)
            {
                if (!(*Iter)->IsAlive())
                    continue;
...
void GrandWidowFaerlinaAI::OnCombatStart(Unit* pTarget)
{
    ...
    for (auto Iter = mWorshippers.begin(); Iter != mWorshippers.end(); ++Iter)
    {
...
void AnubRekhanAI::OnCombatStart(Unit* pTarget)
{
    ...
    if (IsHeroic())
    {
        for (auto Iter = mCryptGuards.begin(); Iter != mCryptGuards.end(); ++Iter)
        {
...
void AnubRekhanAI::OnCombatStop(Unit* pTarget)
{
    ...
    for (auto Iter = mScarabs.begin(); Iter != mScarabs.end(); ++Iter)
    {
        (*Iter)->mAnubRekhanAI = NULL;
        if (IsAlive())
            (*Iter)->Despawn();
...
void SpellFunc_AnubRekhanCorpseScarabsPlayer(SpellDesc* pThis, MoonScriptCreatureAI* pCreatureAI, Unit* pTarget, TargetType pType)
{
    ...
    if (AnubRekhan != NULL)
    {
        ...
        for (auto Iter = AnubRekhan->GetUnit()->GetInRangePlayerSetBegin(); Iter != AnubRekhan->GetUnit()->GetInRangePlayerSetEnd(); ++Iter)
        {
            if ((*Iter) == NULL)
                continue;
...
void SpellFunc_AnubRekhanCorpseScarabsCryptGuard(SpellDesc* pThis, MoonScriptCreatureAI* pCreatureAI, Unit* pTarget, TargetType pType)
{
    ...
    if (AnubRekhan != NULL)
    {
        ...
        for (auto Iter = AnubRekhan->GetUnit()->GetInRangeSetBegin(); Iter != AnubRekhan->GetUnit()->GetInRangeSetEnd(); ++Iter)
        {
...
void NothThePlaguebringerAI::OnCombatStop(Unit* pTarget)
{
    ...
    for (auto Iter = mChampions.begin(); Iter != mChampions.end(); ++Iter)
    {
        (*Iter)->mNothAI = NULL;
        (*Iter)->Despawn();
    };
    ...
    for (auto Iter = mGuardians.begin(); Iter != mGuardians.end(); ++Iter)
    {
        (*Iter)->mNothAI = NULL;
        (*Iter)->Despawn();
    };
...
void HeiganTheUncleanAI::CallEruptionEvent(int32 pTimerId, int32 pNewTime)
{
    ...
    for (auto Iter = mFissures.begin(); Iter != mFissures.end(); ++Iter)
    {
...
void HeiganTheUncleanAI::OnCombatStart(Unit* pTarget)
{
    ...
    if (_unit->GetMapMgr() != NULL && _unit->GetMapMgr()->GetInterface() != NULL)
    {
        ...
        if (mFissures.size() == 0)
        {
            ...
            for (auto Iter = _unit->GetInRangeSetBegin(); Iter != _unit->GetInRangeSetEnd(); ++Iter)
            {
...
{
    for (auto itr = mFissures.begin(); itr != mFissures.end(); ++itr)
    {
...
void PlagueFissureGO::Destroy()
{
    if (mHeiganAI != NULL)
    {
        for (auto Iter = mHeiganAI->mFissures.begin(); Iter != mHeiganAI->mFissures.end(); ++Iter)
        {
            if ((*Iter).second == this)
            {
...
{
    ParentClass::OnCombatStop(pTarget);
    for (auto Iter = mSpores.begin(); Iter != mSpores.end(); ++Iter)
    {
...
void ShadeOfNaxxramasAI::OnDied(Unit* pKiller)
{
    ...
    for (auto Iter = mPortals.begin(); Iter != mPortals.end(); ++Iter)
    {
        (*Iter)->mShadeAI = NULL;
        (*Iter)->Despawn();
    };
...
void SpellFunc_PatchwerkHatefulStrike(SpellDesc* pThis, MoonScriptCreatureAI* pCreatureAI, Unit* pTarget, TargetType pType)
{
    ...
    for (auto PlayerIter = pCreatureAI->GetUnit()->GetInRangePlayerSetBegin();
