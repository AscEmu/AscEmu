...
void NaxxramasWorshipperAI::OnDied(Unit* pKiller)
{
    ...
    if (mGrandWidow != NULL)   //&& !IsHeroic())
    {
        if (GetRange(static_cast< MoonScriptCreatureAI* >(mGrandWidow)) <= 15.0f)
        {
            ...
            Aura* WidowEmbrace = sSpellFactoryMgr.NewAura(sSpellCustomizations.GetServersideSpell(NAXXRAMAS_WORSHIPPER_WIDOW_EMBRACE), 30000, _unit, mGrandWidow->GetUnit());
...
void NaxxramasWorshipperAI::AIUpdate()
{
    /*if (!mPossessed && _unit->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_PLAYER_CONTROLLED_CREATURE))
    {
        ...
        if (PlayerPtr != NULL)
        {
            ...
            NewSpell->spell = sSpellCustomizations.GetServersideSpell(NAXXRAMAS_WORSHIPPER_WIDOW_EMBRACE);
