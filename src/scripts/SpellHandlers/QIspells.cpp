...
bool KarangsBanner(uint32 i, Spell* pSpell)
{
    ...
    // Banner Aura
    pPlayer->CastSpell(pPlayer, sSpellCustomizations.GetServersideSpell(20746), true);
...
bool GoblinWeatherMachine(uint32 i, Spell* pSpell)
{
    ...
    pSpell->p_caster->CastSpell(pSpell->p_caster, sSpellCustomizations.GetServersideSpell(Weather), true);
    return true;
}
...
bool HunterTamingQuest(uint32 i, Aura* a, bool apply)
{
    ...
    else
    {
        ...
        OLD_SpellEntry* triggerspell = sSpellCustomizations.GetServersideSpell(TamingSpellid);
...
bool Triage(uint32 i, Spell* pSpell)
{
    ...
    pSpell->p_caster->CastSpell(pSpell->GetUnitTarget(), sSpellCustomizations.GetServersideSpell(746), true);
...
bool ManaRemnants(uint32 i, Spell* pSpell)
{
    ...
    for (uint8 i = 0; i < 2; i++)
    {
        ...
        if (qle != nullptr && qle->GetMobCount(0) < qle->GetQuest()->required_mob_or_go_count[0])
        {
            pPlayer->CastSpell(Ward, sSpellCustomizations.GetServersideSpell(44981), false);
...
bool Carcass(uint32 i, Spell* pSpell) // Becoming a Shadoweave Tailor
{
    ...
    {
        NetherDrake->CastSpell(NetherDrake, sSpellCustomizations.GetServersideSpell(38502), true);
...
bool ForceofNeltharakuSpell(uint32 i, Spell* pSpell) // Becoming a Shadoweave Tailor
{
    ...
    if (pTarget->GetEntry() == 21722 && pPlayer->CalcDistance(pTarget) < 30)
    {
        if (pQuest->GetMobCount(0) < pQuest->GetQuest()->required_mob_or_go_count[0])
        {
            pTarget->CastSpell(pPlayer, sSpellCustomizations.GetServersideSpell(38775), true);
            pQuest->SetMobCount(0, pQuest->GetMobCount(0) + 1);
            pQuest->SendUpdateAddKill(0);
            pQuest->UpdatePlayerFields();
            if (1)
