...
bool HotStreak(uint32 i, Aura* pAura, bool apply)
{
    ...
    if (i == 0)
    {
        ...
        if (apply)
        {
            ...
            caster->AddProcTriggerSpell(48108, pAura->GetSpellInfo()->getId(), caster->GetGUID(), pAura->GetSpellInfo()->getEffectBasePoints(i) + 1, PROC_ON_SPELL_CRIT_HIT | PROC_ON_SPELL_HIT, 0, pAura->GetSpellInfo()->EffectSpellClassMask[i], classMask);
        }
        else
