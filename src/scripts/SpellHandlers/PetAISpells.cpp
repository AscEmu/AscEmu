        ...
        void OnLoad()
        {
            if (_unit->IsSummon())
            {
                ...
                if (_unit->GetCreatedBySpell() == 58833)
                {
                    ...
                    AI_Spell sp1;
                    sp1.entryId = 59638;
                    sp1.spell = sSpellCustomizations.GetServersideSpell(sp1.entryId);
                    if (!sp1.spell)
                        return;
                    ...
                    AI_Spell sp2;
                    sp2.entryId = 59637;
                    sp2.spell = sSpellCustomizations.GetServersideSpell(sp2.entryId);
                    if (!sp2.spell)
                        return;
    ...
    public:
        ...
        void AIUpdate()
        {
            ...
            if (_unit->GetCurrentSpell() == NULL && curtarget)
            {
                ...
                OLD_SpellEntry* MyNextSpell = sSpellCustomizations.GetServersideSpell(dpsSpell);
                if (MyNextSpell != NULL)
        ...
        void OnHit(Unit* mTarget, float fAmount)
        {
            for (uint8 p = 0; p < 5; p++)
            {
                if (procSpell[p] != 0)
                {
                    OLD_SpellEntry* mProc = sSpellCustomizations.GetServersideSpell(procSpell[p]);
                    if (!mProc)
                        return;
                    int x = RandomUInt(99);
