...
class LuaUnit
{
    ...
    static int CastSpell(lua_State* L, Unit* ptr)
    {
        ...
        if (sp && ptr)
            ptr->CastSpell(ptr, sSpellCustomizations.GetServersideSpell(sp), true);
        return 0;
    }
    ...
    static int FullCastSpell(lua_State* L, Unit* ptr)
    {
        uint32 sp = CHECK_ULONG(L, 1);
        if (sp && ptr)
            ptr->CastSpell(ptr, sSpellCustomizations.GetServersideSpell(sp), false);
        return 0;
    }
    ...
    static int CastSpellAoF(lua_State* L, Unit* ptr)
    {
        ...
        if (!sp || !ptr)
            return 0;
        ptr->CastSpellAoF(x, y, z, sSpellCustomizations.GetServersideSpell(sp), true);
        return 0;
    }
    ...
    static int FullCastSpellAoF(lua_State* L, Unit* ptr)
    {
        ...
        if (!sp || !ptr)
            return 0;
        ptr->CastSpellAoF(x, y, z, sSpellCustomizations.GetServersideSpell(sp), false);
        return 0;
    }
    ...
    static int Strike(lua_State* L, Unit* ptr)
    {
        ...
        if (!target)
            return 0;
        ptr->Strike(target, weapon_damage_type, sSpellCustomizations.GetServersideSpell(sp), adddmg, pct_dmg_mod, exclusive_damage, false, false);
        return 0;
    }
    ...
    static int EventCastSpell(lua_State* L, Unit* ptr)
    {
        ...
        if (ptr && sp)
        {
            ...
            {
                case TYPEID_PLAYER:
                    sEventMgr.AddEvent(ptr, &Player::EventCastSpell, target, sSpellCustomizations.GetServersideSpell(sp), EVENT_PLAYER_UPDATE, delay, repeats, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
                    break;
                case TYPEID_UNIT:
                    sEventMgr.AddEvent(ptr, &Unit::EventCastSpell, target, sSpellCustomizations.GetServersideSpell(sp), EVENT_CREATURE_UPDATE, delay, repeats, EVENT_FLAG_DO_NOT_EXECUTE_IN_WORLD_CONTEXT);
                    break;
            }
        }
        return 0;
    ...
    static int AddAura(lua_State* L, Unit* ptr)
    {
        ...
        if (ptr && spellid)
        {
            Aura* aura = sSpellFactoryMgr.NewAura(sSpellCustomizations.GetServersideSpell(spellid), duration, ptr, ptr, temp);
            ptr->AddAura(aura);
            lua_pushboolean(L, 1);
        }
        else
    ...
    static int GetGuildName(lua_State* L, Unit* ptr)
    {
        ...
        if (pGuild != NULL)
            lua_pushstring(L, pGuild->GetName().c_str());
        else
            lua_pushnil(L);
        return 1;
    }
    ...
    static int ChannelSpell(lua_State* L, Unit* ptr)
    {
        ...
        if (Csp && target != NULL)
        {
            ptr->CastSpell(target->GetGUID(), sSpellCustomizations.GetServersideSpell(Csp), false);
    ...
    */
    ...
    static int SendPacketToGuild(lua_State* L, Unit* ptr)
    {
        ...
        if (g_id >= 0)
        {
            guild = sGuildMgr.GetGuildById(g_id);
        }
        else
        {
            guild = plr->GetGuild();
    ...
    static int GetGuildRank(lua_State* L, Unit* ptr)
    {
        TEST_PLAYER()
            Player* plr = static_cast<Player*>(ptr);
        if (plr->GetGuild() != NULL)
            lua_pushinteger(L, plr->GetRankFromDB(plr->GetGUID()));
        else
            lua_pushnil(L);
        return 1;
    }
    ...
    static int GetGuildMotd(lua_State* L, Unit* ptr)
    {
        ...
        if (g_id >= 0)
            ...
            guild = plr->GetGuild();
        if (guild != NULL)
            lua_pushstring(L, guild->GetMOTD().c_str());
        else
            lua_pushnil(L);
        return 1;
    }
    ...
    static int SetGuildInformation(lua_State* L, Unit* ptr)
    {
        ...
        if (gi && plr->GetGuild())
            plr->GetGuild()->HandleSetMOTD(plr->GetSession(), gi);
        return 0;
    }
    ...
    static int RemoveGuildMember(lua_State* L, Unit* ptr)
    {
        ...
        Player* target = CHECK_PLAYER(L, 1);
        if (target && plr->GetGuild())
            plr->GetGuild()->HandleRemoveMember(plr->GetSession(), target->GetGUID());
        return 0;
    }
    ...
    static int SetOfficerNote(lua_State* L, Unit* ptr)
    {
        ...
        const char* note = luaL_checkstring(L, 2);
        if (target && note && plr->GetGuild())
            plr->GetGuild()->HandleSetMOTD(plr->GetSession(), note);
        return 0;
    }
    ...
    static int GuildBankDepositMoney(lua_State* L, Unit* ptr)
    {
        ...
        if (plr->GetGuild() != NULL)
            plr->GetGuild()->HandleMemberDepositMoney(plr->GetSession(), amount);
        return 0;
    }
    ...
    static int GuildBankWithdrawMoney(lua_State* L, Unit* ptr)
    {
        ...
        uint32 amount = luaL_checkinteger(L, 1);
        if (plr->GetGuild() != NULL)
            plr->GetGuild()->HandleMemberWithdrawMoney(plr->GetSession(), amount);
        return 0;
    }
    ...
    static int GetGuildLeader(lua_State* L, Unit* ptr)
    {
        ...
        if (pGuild != nullptr)
        {
            Player* plr = objmgr.GetPlayer(Arcemu::Util::GUID_LOPART(pGuild->GetLeaderGUID()));
            if (plr != nullptr)
